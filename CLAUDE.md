...
| `benchmarks/`      | Anonymous peer benchmarks across opportunities: cohorts (membership IS the read grant), per-cohort disclosure settings (currently all off for KMC), publications of point values plus tenure-axis trends from the source report's saved runs, auto-republish when that report saves a run. Read by `kmc_opp_report`. **Read [connect_labs/benchmarks/README.md](connect_labs/benchmarks/README.md) first.** | `models.py`, `publish.py`, `disclosure.py`, `auto_publish.py`, `mcp_tools.py`, `tasks.py` |
...
| `marketplace/`     | **Org directory at `/labs/marketplace/`.** `LabsOrg` is the MASTER org registry — an org that answered an EOI and lost never reaches Connect, so this is strictly larger than Connect's org set. Satellites (`OrgProfile`, `OrgContact`, `OrgConnectSlug`) + EOI rounds/submissions (`solicitations.local_models`, `llo_entity` is a real FK) are fed from the LLO Directory sheet by `marketplace_import [--eoi] [--check-access]`. Pulse partner identity reads from here. Matching is exact-only; anything else waits for a human verdict on the sheet's `EOI Response Mapping` tab. Every import publishes a row-numbered data-quality report to the `Labs Findings` tab. The **canopy agent panel** rides the network and round pages — see [docs/canopy-agent-panel.md](docs/canopy-agent-panel.md). | `models.py`, `directory.py`, `identity.py`, `eoi.py`, `matching.py`, `responses.py`, `quality.py`, `views.py` |
| `semantic/`        | SQL semantic layer — a general indicator engine: a YAML/record registry compiled to SQL and run in-process over the visit cache. Each registry declares its own **model** (`entity`, `visit_columns`, `pipelines`, optional `weight_series`, `defaults`, `series`) in `model.py` terms; KMC (`registry/kmc`, the only source for the KMC dashboards' indicators since the browser engine was deleted; one set of slug ids since #2004, Neal Lesh's compute spec rules throughout) and `registry/visit_quality` are two examples. `legacy.py` supplies KMC's model ONLY to records saved before the model existed. Every SQL fragment is allow-list checked on save and on compile. Served at `workflow/api/<id>/semantic/`. User docs: `user_docs/semantic-layer.md`; parity: `connect_labs/semantic/PARITY.md`. | `model.py`, `legacy.py`, `compiler.py`, `layer1.py`, `runtime.py`, `snapshot.py`, `validation.py`, `workflow_binding.py`, `registry/{kmc,visit_quality}/*.yml` |
...
**Existing templates** (35; `*` = multi-opp): `audit_par`\*, `audit_with_ai_review`, `bulk_image_audit`, `chc_audit_history`\*, `chc_nutrition_analysis`, `flw_audit_trend_dashboard`\*, `flw_daily_indicator_report`\*, `flw_daily_indicator_table`\*, `flw_daily_summary_report`\*, `flw_daily_summary_report_rutf`\*, `flw_weekly_audit_report`\*, `indicator_opp_report`, `indicator_programme_report`\*, `indicator_worker_review`\*, `interviews_reporting_v2`, `jakusko_chlorine_dispenser`, `kmc_flw_flags`, `kmc_flw_review`\*, `kmc_image_audit`\*, `kmc_longitudinal`, `kmc_opp_report`, `kmc_programme_metrics`\*, `kmc_project_metrics`, `llo_weekly_review`, `mbw_auditing_v5`, `mbw_auditing_v6`, `mbw_visit_verification`, `muac_picture_audit`\*, `ocs_outreach`, `performance_review`\*, `program_admin_report`\*, `program_audit_creator`\*, `sam_followup`, `verified_monitoring`, `weekly_dual_track_audit`\*

**The KMC report family** is the worked example of reports that stay in step. `kmc_programme_metrics` (the programme report, saved weekly runs, semantic-layer snapshot) drills into `kmc_flw_review` (its companion, one worker). `kmc_opp_report` is the programme report's content for one opportunity, for its network manager, plus anonymous peer benchmarks, fanned out one per opportunity by `benchmarks_create_opp_reports`; it has saved runs of its own and receives the programme report's saved weeks cut to its opportunity (`workflow/hand_down.py`). Both reports draw with the shared report library `window.LabsReport` (`components/workflow/report/`), which any render can use at runtime. Every instance follows the deployed template (`render_source`), shares the programme report's pipelines and live registry record, resolves config from the template on read, and fills its own visit cache when cold (`warm_cache_on_read`). **The generic indicator cascade** — `indicator_programme_report` (companion `indicator_worker_review`, hands down to `indicator_opp_report`) — is the same family for ANY registry: one render (`indicator_report_render.js`) for the programme and opportunity reports, no programme-specific code, every tile/target/label/noun/scorecard column from the registry's display contract (`semantic/display.py`, `user_docs/semantic-layer.md#how-a-report-reads-it-display`). Its snapshot spec names only the builder; `snapshot_builders.resolve_spec_defaults` derives scopes, case index, visit pipeline and credibility from the registry. Instances follow the template from creation (`follow_template`). Read [WORKFLOW_REFERENCE.md §12](connect_labs/workflow/WORKFLOW_REFERENCE.md#12-reports-that-stay-in-step-followed-templates-fan-out-warm-on-read) before adding a per-instance copy of anything, and [connect_labs/benchmarks/README.md](connect_labs/benchmarks/README.md) before publishing a benchmark (rebuild history first, then publish).
...
registers **205 tools** (write tools are rate-limited and fully argument-logged
to `MCPAuditLog`) — 83 of them generated from the supply-chain
...
86: three are `internal=True` (`catalogue_seed`, `tracker_import`,
...
- **[docs/canopy-agent-panel.md](docs/canopy-agent-panel.md)** — the canopy agent panel on the marketplace pages: labs signs an Ed25519 assertion for the signed-in visitor, the widget hands the agent the SELECTION on screen (slugs, never rows) and it reads the rows back through `marketplace_orgs_get`. Three labs-specific traps: the CSRF cookie is `HttpOnly` so the token comes from the DOM; the include belongs in `inline_javascript` because `javascript` renders in `<head>` where there is no `document.body`; and the agent's tools run as the AGENT, not as the visitor
