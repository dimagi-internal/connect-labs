...
| `supply_chain/`    | Core-labs supply domain at `/supply/`: procurement (RUTF quoting/awards) is its first sub-component, with tracking and distribution as named slots to follow. Reference data (commodities, suppliers) is org-scoped; procurement records are program-scoped. See `docs/superpowers/specs/2026-09-11-rutf-procurement-design.md`. | `records.py` (LabsRecord type constants), `management/commands/supply_scope_probe.py` |
