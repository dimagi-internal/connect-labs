"""
RUTF pipeline configuration for extracting visit-level data.

Uses the labs analysis pipeline to extract MUAC measurements, photos, and other metrics
from RUTF/SAM follow-up visits at the visit level (not child level - aggregation happens in views).
"""

# Import timeline config to reuse field definitions (DRY principle)
from connect_labs.custom_analysis.rutf.timeline_config import RUTF_HEADER_FIELDS, RUTF_WIDGETS
from connect_labs.labs.analysis import AnalysisPipelineConfig, CacheStage, FieldComputation
...
def _is_valid_muac(x) -> bool:
    """Check if value is a valid MUAC measurement (numeric)."""
    if not x:
        return False
    s = str(x).strip()
...
def _get_transform_function(transform_name: str):
    ...
    Args:
        transform_name: String like "float", "date"

    Returns:
        Transform function for FieldComputation
    """
    if transform_name == "float":
        return lambda x: float(x) if x and _is_valid_muac(x) else None
...
def _create_field_computation_from_extractor(field_name: str, extractor, description: str = None):
    ...
    """
    # Use 'paths' for multiple paths (tries each in order), 'path' for single path
    if len(extractor.form_paths) > 1:
        return FieldComputation(
            name=field_name,
            paths=extractor.form_paths,  # Multiple paths - try each in order
            aggregation="first",
...
def _build_widget_fields():
    ...
    for widget_id, widget_config in RUTF_WIDGETS.items():
...
def _build_header_fields():
    ...
    return [
        ...
        for field_name, extractor in RUTF_HEADER_FIELDS.items()
    ]


RUTF_PIPELINE_CONFIG = AnalysisPipelineConfig(
    grouping_key="username",
    experiment="rutf_timeline",
    terminal_stage=CacheStage.VISIT_LEVEL,  # Visit-level, not aggregated
    # Use case_id for linking - unique per household/child case
    linking_field="rutf_case_id",
    fields=[
        # Case ID - unique per child case, used for linking visits
        FieldComputation(
            name="rutf_case_id",
            paths=[
                "form.case.@case_id",  # Case ID from form
            ],
            aggregation="first",
            description="Case ID (unique per child, used for linking)",
        ),
        # Entity ID from deliver unit
        FieldComputation(
            name="entity_id",
            paths=[
                "form.sam_followup.deliver.entity_id",
            ],
            aggregation="first",
        ...
        FieldComputation(
            name="entity_name",
            paths=[
                "form.sam_followup.deliver.entity_name",
            ],
            aggregation="first",
            description="Entity name from deliver unit",
        ),
        # ===== Fields generated from timeline config (DRY) =====
        # Widget fields (muac, date, time_end, visit_number, etc.)
        *_build_widget_fields(),
        # Header fields (child_name, child_dob, child_gender, household_name, etc.)
