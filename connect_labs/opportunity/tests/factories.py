...
from connect_labs.commcarehq.tests.factories import HQServerFactory
from connect_labs.opportunity.models import Country, Currency, VisitValidationStatus
from connect_labs.users.tests.factories import OrganizationFactory
...
class HQApiKeyFactory(DjangoModelFactory):
    api_key = Faker("uuid4")
    user = SubFactory("connect_labs.users.tests.factories.UserFactory")
    hq_server = SubFactory(HQServerFactory)
...
class OpportunityAccessFactory(DjangoModelFactory):
    ...
    user = SubFactory("connect_labs.users.tests.factories.MobileUserFactory")
...
class PaymentUnitFactory(DjangoModelFactory):
    ...
    # parent_payment_unit = SubFactory("connect_labs.opportunity.tests.factories.PaymentUnitFactory")
