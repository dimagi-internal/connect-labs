...
**Status:** Active
**Date:** 2026-06-19
**Owner doc location:** This file lives **inside the app** (`connect_labs/campaign/`) by design. The
...
labs-specific test scaffolding**. If you move `connect_labs/campaign/` somewhere else, the tests in
...
2. **Self-contained & portable.** All fixtures, factories, test settings, and browser tests live under
   `campaign/tests/`. No test may depend on `connect_labs/conftest.py`, labs factories, or
...
pytest connect_labs/campaign/
# contract layer only
pytest connect_labs/campaign/ -m contract
# browser suite (pre-deploy / nightly), against a running instance
pytest connect_labs/campaign/tests/e2e/ --base-url=https://labs.connect.dimagi.com
```
