...
from ..models import Bid, EOIRound, EOISubmission, RFP
...
def _supplier_world(actor):
    ...
    my_subs = (
        EOISubmission.objects.filter(org=org).select_related("round", "org").order_by("-created_at")
    )
    ...
    my_bids = {
        b.rfp_id: b
        for b in Bid.objects.filter(org=org).prefetch_related("lot_bids__bid__org")
    }
    rfps = []
    for rfp in eligible:
        data = rfp_dict(rfp)
    ...
    return {
        "org": org_dict(org),
        "open_rounds": [
            {**round_dict(r), "applied": r.id in submitted_round_ids} for r in open_rounds
        ],
...
def _staff_world(actor):
    ...
    if "rfps" in ROLE_PERMS.get(role, {}):
        world["rfps"] = [
            rfp_dict(r) for r in RFP.objects.prefetch_related("lots").all().order_by("-created_at")
        ]
    return world
