...
def test_signup_creates_user_org_membership():
    resp = Client().post("/oes/signup/", SIGNUP)
...
def test_login_logout_flow():
    Client().post("/oes/signup/", SIGNUP)
    c = Client()
    resp = c.post("/oes/login/", {"email": SIGNUP["email"], "password": "correct-horse-9"})
    assert resp.status_code == 302
    assert resp.url == "/oes/"
    assert c.post("/oes/logout/").status_code == 302
    # after logout the SPA shell bounces to login
    assert c.get("/oes/").url == "/oes/login/"


def test_signup_duplicate_email_shows_error():
    Client().post("/oes/signup/", SIGNUP)
    resp = Client().post("/oes/signup/", {**SIGNUP, "org_name": "Other Org"})
...
def test_signup_duplicate_org_name_shows_error():
    Client().post("/oes/signup/", SIGNUP)
    resp = Client().post("/oes/signup/", {**SIGNUP, "email": "other@x.example"})
    assert resp.status_code == 200
    assert b"already" in resp.content.lower()
...
def test_signup_rejects_privileged_domains(email):
    ...
    resp = Client().post("/oes/signup/", {**SIGNUP, "email": email})
    assert resp.status_code == 200
...
def test_signup_password_mismatch():
    resp = Client().post("/oes/signup/", {**SIGNUP, "password2": "different-9"})
    assert resp.status_code == 200
...
def test_login_bad_password():
    Client().post("/oes/signup/", SIGNUP)
    resp = Client().post("/oes/login/", {"email": SIGNUP["email"], "password": "wrong"})
    assert resp.status_code == 200
    assert b"invalid" in resp.content.lower()


def test_login_page_renders_for_anonymous():
    assert Client().get("/oes/login/").status_code == 200
    assert Client().get("/oes/signup/").status_code == 200
