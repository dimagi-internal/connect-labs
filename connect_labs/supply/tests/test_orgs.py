...
def test_supplier_reads_and_edits_own_profile(supplier_client):
    ...
    resp = client.get("/oes/api/org/profile/")
...
def test_certifications_add_and_delete(supplier_client):
    ...
    assert _post(client, f"/oes/api/org/certifications/{cert_id}/delete/", {}).status_code == 200
...
def test_cannot_touch_another_orgs_certification(supplier_client):
    ...
    resp = _post(client, f"/oes/api/org/certifications/{rival_cert.id}/delete/", {})
...
def test_staff_cannot_use_supplier_profile_endpoints(admin_client):
    ...
    assert client.get("/oes/api/org/profile/").status_code == 403


def test_anonymous_gets_401(db):
    from django.test import Client

    assert Client().get("/oes/api/org/profile/").status_code == 401
