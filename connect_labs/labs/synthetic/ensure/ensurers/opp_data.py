...
1. **Register the labs-only synthetic opportunity** for ``resource.opportunity_id``
   so :func:`connect_labs.labs.synthetic.registry.get_synthetic_opp` returns an
   enabled :class:`~connect_labs.labs.synthetic.models.SyntheticOpportunity`. The
...
2. **Load + validate the per-opp generator** ``Manifest`` (from
   ``connect_labs.labs.synthetic.generator.fixtures.manifest``) from
