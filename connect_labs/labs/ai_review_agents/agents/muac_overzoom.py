...
class MUACOverzoomAgent(BaseAIReviewAgent):
    ...
    def review(self, context: ReviewContext) -> ReviewResult:
        ...
        try:
            ...
            return ReviewResult.error(GATEWAY_UNEXPECTED_RESPONSE_MESSAGE, error_kind=ERROR_KIND_UNEXPECTED_RESPONSE)
            ...
            self.logger.error(f"MUAC overzoom API error: status={e.response.status_code} {error_detail}")
        ...
        # costs a full client timeout of wall-clock per image.
            ...
            self.logger.error(f"MUAC overzoom timeout after {self.DEFAULT_TIMEOUT}s: {e}")
