...
def test_preview_frame_enqueues(client, django_user_model, monkeypatch):
    _login(client, django_user_model)
    from connect_labs.microplans.tasks import generate_frame_task
...
def test_compare_surrounding_enqueues(client, django_user_model, monkeypatch):
    _login(client, django_user_model)
    from connect_labs.microplans.tasks import compare_surrounding_wards_task
...
def test_preview_coverage_enqueues(client, django_user_model, monkeypatch):
    _login(client, django_user_model)
    from connect_labs.microplans.tasks import generate_coverage_task
...
def test_preview_footprints_enqueues(client, django_user_model, monkeypatch):
    _login(client, django_user_model)
    from connect_labs.microplans.tasks import fetch_footprints_task
...
def test_generate_frame_task_returns_envelope(monkeypatch):
    monkeypatch.setattr("connect_labs.microplans.tasks.set_task_progress", lambda *a, **k: None)
    fake = FrameResult(
        pins_geojson={
    ...
    monkeypatch.setattr("connect_labs.microplans.sampling.frame.generate_frame", lambda areas, config: fake)
    from connect_labs.microplans.tasks import generate_frame_task
...
def test_generate_frame_task_maps_value_error_to_envelope(monkeypatch):
    ...
    monkeypatch.setattr("connect_labs.microplans.sampling.frame.generate_frame", too_big)
    from connect_labs.microplans.tasks import generate_frame_task
...
def test_generate_frame_task_propagates_unexpected(monkeypatch):
    ...
    monkeypatch.setattr("connect_labs.microplans.sampling.frame.generate_frame", boom)
    from connect_labs.microplans.tasks import generate_frame_task
...
def test_fetch_footprints_task_emits_polygons_with_centroid_fallback(monkeypatch):
    ...
    monkeypatch.setattr("connect_labs.microplans.core.footprints.fetch_buildings", fake_fetch)
    from connect_labs.microplans.tasks import fetch_footprints_task
...
def test_generate_coverage_task_returns_envelope(monkeypatch):
    monkeypatch.setattr("connect_labs.microplans.tasks.set_task_progress", lambda *a, **k: None)
    from connect_labs.microplans.coverage.frame import CoverageFrameResult
    ...
    monkeypatch.setattr("connect_labs.microplans.coverage.frame.generate_coverage_frame", lambda areas, config: fake)
    from connect_labs.microplans.tasks import generate_coverage_task
...
def _make_fake_program_da(monkeypatch, plans=None, groups=None):
    ...
    monkeypatch.setattr("connect_labs.microplans.core.data_access.ProgramPlanDataAccess", FakeDA)
    return plans, groups
...
def test_apply_plan_mutation_task_dispatches_and_returns_plan_json(monkeypatch):
    ...
    monkeypatch.setattr("connect_labs.microplans.core.data_access.ProgramPlanDataAccess", FakeDA)
    monkeypatch.setattr("connect_labs.microplans.serialization.plan_to_json", lambda p: {"status": "ok", "plan": p})
...
def test_program_group_generate_enqueues_with_group_id(client, django_user_model, monkeypatch):
    ...
    _make_fake_program_da(monkeypatch, {}, groups)
    from connect_labs.microplans.tasks import generate_group_samples_task
...
def test_program_compare_uses_list_plans_not_per_id(client, django_user_model, monkeypatch):
    ...
    monkeypatch.setattr("connect_labs.microplans.core.data_access.ProgramPlanDataAccess", CountDA)
...
def test_program_plan_delete_foreign_record_returns_404(client, django_user_model, monkeypatch):
    ...
    monkeypatch.setattr("connect_labs.microplans.core.data_access.ProgramPlanDataAccess", RefuseDA)
...
def test_program_plan_get_returns_404_when_plan_missing(client, django_user_model, monkeypatch):
    ...
    monkeypatch.setattr("connect_labs.microplans.core.data_access.ProgramPlanDataAccess", _NoneDA)
...
def test_preview_service_delivery_merges_colored_layers(client, django_user_model, monkeypatch):
    ...
    monkeypatch.setattr("connect_labs.microplans.service_delivery.points.fetch_points", fake_fetch)
    resp = client.post(
        reverse("microplans:preview_service_delivery", kwargs={"opp_id": 100}),
...
def test_preview_service_delivery_caps_overlay_points(client, django_user_model, monkeypatch):
    from connect_labs.microplans.service_delivery.points import MAX_OVERLAY_POINTS
...
def test_program_plan_footprints_sets_cache_control(client, django_user_model, monkeypatch):
    ...
    monkeypatch.setattr(
        "connect_labs.microplans.core.footprints.fetch_buildings",
...
def test_boundary_viewport_bbox_snaps_to_grid():
    ...
    from connect_labs.microplans.views import BoundaryViewportView
...
class TestProgramMapSeed:
    ...
    def test_centroids_and_country(self):
        from connect_labs.microplans.views import _program_map_seed
...
def test_adm1_state_for_matches_containing_boundary(monkeypatch):
    ...
    from connect_labs.microplans import views
    ...
    monkeypatch.setattr("connect_labs.labs.admin_boundaries.models.AdminBoundary", FakeManager, raising=False)
...
def test_filter_demo_junk_opps_excludes_test_entries():
    ...
    from connect_labs.microplans.views import _filter_demo_junk_opps
