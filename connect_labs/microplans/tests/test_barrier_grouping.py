...
def test_no_group_spans_a_barrier():
    ...
    cfg = G.GroupingConfig(strategy="barrier_aware", target_buildings=1000, buffer_distance_m=200)
...
def test_barrier_forces_split_even_under_cap():
    ...
    cfg = G.GroupingConfig(strategy="barrier_aware", target_buildings=200, buffer_distance_m=200)
    G.group_work_areas(was, cfg, barriers=river)
...
def test_top_up_pass_never_merges_or_steals_across_a_barrier():
    # c0..c3 (200 total) on the near bank, c4..c5 (100) stranded on the far bank
    # by the river — even though c4..c5 is below min_buildings and geographically
    # close, the top-up pass must not cross the SAME barrier phase-1 respected.
    was = [_cell(i, buildings=50) for i in range(6)]
    river = LineString([(0.00385, -1), (0.00385, 1)])  # between c3 and c4
    cfg = G.GroupingConfig(
        strategy="barrier_aware",
        target_buildings=1000,  # no phase-1 cap split — only the river splits them
    ...
    G.group_work_areas(was, cfg, barriers=river)
    groups = _groups(was)
    left, right = {"c0", "c1", "c2", "c3"}, {"c4", "c5"}
    spanning = [g for g, ids in groups.items() if set(ids) & left and set(ids) & right]
    assert spanning == [], f"top-up pass merged/stole across the river: {groups}"
    # The far-bank group is below min_buildings (100 < 150) but correctly left
    # standalone — there's nothing on its own side of the river to top up from.
    far_group = next(g for g, ids in groups.items() if set(ids) == right)
    assert sum(50 for _ in groups[far_group]) == 100
