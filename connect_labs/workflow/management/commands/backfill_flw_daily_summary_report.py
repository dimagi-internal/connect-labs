"""One-off backfill for the flw_daily_summary_report template's daily runs.
...
identical logic. Mirrors backfill_flw_daily_indicator_report.py exactly (this
template needs no CommCare HQ token -- both pipelines are connect_csv and the
photo-taken indicators' extra fetch uses the same Connect access token as
everything else).
...
run_scheduled_workflow uses.

Not idempotent by default: every call creates a fresh WorkflowRun per
opportunity per day, matching flw_daily_indicator_report's own "Fire =
execute, no reuse" convention. Pass --replace-existing to first delete any
run(s) already present for the same (definition, period_start) before
creating the new one.

Intended to be invoked once via the run-labs-command.yml GitHub Action
against production, e.g.:

    backfill_flw_daily_summary_report --definition <id> --program 217 \\
...
class Command(BaseCommand):
    help = "Backfill flw_daily_summary_report runs for the last N completed WAT calendar days."
