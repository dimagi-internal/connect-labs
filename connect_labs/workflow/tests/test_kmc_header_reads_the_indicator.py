...
(2026-09-11) that read 8,850 babies above a scorecard whose total said 8,823 --
...
def test_the_headline_count_comes_from_total_cases_not_the_case_rows():
    flat = _flat()
    assert "functiontotalCases(ind,fallback)" in flat, "the indicator-first helper must exist"
    assert "ind['total_cases']" in RENDER.read_text()
    assert "nCount(totalCases(programInd,meta.cases))" in flat, (
        "the programme subline must read the total_cases indicator, falling back to meta.cases "
...
def test_one_organisation_headlines_the_same_measure_as_the_programme():
    """Registered cases is a different number. Using it here made the word 'babies' mean a
    ...
    assert "entryOf(scopeLLO&&scopeLLO.ind,'registered_cases')" not in flat
