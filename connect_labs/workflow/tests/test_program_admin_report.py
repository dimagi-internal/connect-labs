...
def _patch_wda_with(side_effect, monkeypatch):
    ...
    """
    from connect_labs.workflow import data_access as wda_module
...
def test_get_saved_runs_for_program_report_filters_by_source_and_window(monkeypatch):
    ...
    from connect_labs.workflow.data_access import get_saved_runs_for_program_report
...
def _patch_path(name):
    return f"connect_labs.workflow.templates.program_admin_report.{name}"
...
def test_rollup_joins_flags_audits_tasks_by_flw(fake_run, monkeypatch):
    ...
    from connect_labs.workflow.templates import program_admin_report as par
    ...
    monkeypatch.setattr("connect_labs.audit.data_access.AuditDataAccess", mock_ada_class)
...
def test_program_admin_report_has_no_build_snapshot_hook():
    ...
    from connect_labs.workflow.templates import TEMPLATES, resolve_snapshot_contract
...
def test_rollup_job_handler_persists_state_and_returns_results(monkeypatch):
    ...
    from connect_labs.workflow.job_handlers.program_admin_report import program_admin_rollup
    ...
    monkeypatch.setattr(
        "connect_labs.workflow.data_access.WorkflowDataAccess",
        MagicMock(return_value=fake_wda),
    )
    monkeypatch.setattr(
        "connect_labs.workflow.templates.program_admin_report.compute_program_admin_rollup",
