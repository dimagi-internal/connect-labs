...
function checks(file, src, ast) {
  test(`${file}: the render parses as JSX`, () => {
    assert.doesNotThrow(ast);
  });

  test(`${file}: every referenced name resolves`, () => {
  ...
  test(`${file}: no top-level declaration appears twice`, () => {
  ...
  test(`${file}: every memo that reads the payload-carried registry facts depends on the payload`, () => {
