...
from connect_labs.workflow.templates.mbw_monitoring.data_transforms import (
    build_gps_visit_dicts,
    check_app_version,
    compute_ebf_by_flw,
    extract_per_mother_fields,
)
from connect_labs.workflow.templates.mbw_monitoring.flw_status import (
    get_latest_flw_statuses as _shared_get_latest_flw_statuses,
)
from connect_labs.workflow.templates.mbw_monitoring.followup_analysis import (
...
from connect_labs.workflow.templates.mbw_monitoring.gps_analysis import (
    analyze_gps_metrics,
    build_result_from_analyzed_visits,
    compute_median_meters_per_visit,
    compute_median_minutes_per_visit,
)
from connect_labs.workflow.templates.mbw_monitoring.pipeline_config import MBW_GPS_PIPELINE_CONFIG
from connect_labs.workflow.templates.mbw_monitoring.serializers import (
    filter_visits_by_date,
    serialize_flw_summary,
    serialize_visit,
)
from connect_labs.workflow.templates.mbw_monitoring.session_adapter import (
    VALID_FLW_RESULTS,
    complete_monitoring_run,
    load_monitoring_run,
    save_dashboard_snapshot,
)
from connect_labs.workflow.templates.mbw_monitoring.session_adapter import save_flw_result as save_flw_result_helper
...
def _get_latest_flw_statuses(request, active_usernames):
    """Backwards-compatible wrapper around the shared FLW status helper.

    The implementation moved to mbw_monitoring/flw_status.py so the V2 job
    handler can call it server-side without pulling in views.py's full
    Django/OAuth dependency chain. Kept as a private alias here so existing
    call sites in this file don't need to change.
    """
    return _shared_get_latest_flw_statuses(request, active_usernames)
