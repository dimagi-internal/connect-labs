...
// V6 compute helpers.
//
// v5 ported v4's Python job handler to JS wholesale: pure functions
// (no React state) matching v4's algorithms line-for-line, including a
// chronological scan over EVERY visit the opportunity has ever recorded to
// build mother→FLW attribution and ANC eligibility — on every run creation,
// in the browser. See mbw_auditing_v6.py's module docstring for the full
// rationale; short version: v6 moves that scan into a SQL entity pipeline
// (`mothers`, one row per mother, computed once and cached) for TAB 1.
//
// TAB 1 (no task_filters): v6_processVisitsSlim (GPS/duration/gap only) +
// v6_processMothers (attribution/ANC/completion from the `mothers` pipeline)
// + v6_computeFollowupRatesFromMothers.
//
// TAB 2 (task_filters set — a per-FLW audit-launch trigger date): the
// `mothers` pipeline is presence-as-of-now only and can't answer an
// arbitrary historical date, so this still runs the full v5/v4-style
// chronological scan (v6_processVisitsFull, v6_computeFollowupRates,
// v6_computeBaselineFollowupRates) — but the caller pre-filters
// visits/registrations down to just the FLWs being audited this run first
// (see computeTab2ByUser), which is what actually shrinks Tab 2's cost.
//
// Shared invariants (both tabs, same as v5/v4):
//   - visits sorted chronologically ASC so last-write-wins mother→FLW
//     attribution matches v4's `mother_to_flw[mid] = username` — and,
//     for Tab 1, the SQL entity stage's `"aggregation": "last"`, which is
//     ALREADY ordered by (visit_date, visit_id) (WORKFLOW_REFERENCE.md §2).
//   - form_name normalized via _FORM_NAME_ALIASES before keying into the
//     visits_by_mother map (matches v4 line 224-225); the `mothers` pipeline
//     carries the one known alias (Postnatal Delivery Visit) as a second
//     field, coalesced client-side.
...
var V6_GRACE_PERIOD_DAYS = 5;
var V6_NON_ANC_VISIT_TYPES = [
...
function v6_normFormName(rawName) {
  var trimmed = (rawName || '').replace(/^\s+|\s+$/g, '');
  return V6_FORM_NAME_ALIASES[trimmed] || trimmed;
}

function v6_dateStr(s) {
  // Slice to YYYY-MM-DD. v4 does this with `[:10]`.
  return (s || '').slice(0, 10);
}

function v6_subtractDays(dateStr, days) {
...
function v6_todayStr(currentDateOverride) {
  if (currentDateOverride) return v6_dateStr(currentDateOverride);
  var n = new Date();
...
// FULL per-visit scan: single chronological pass building all per-FLW and
// per-mother indices. Mirrors v4 handler lines 174-290.
//
// v6 only calls this for TAB 2 (task_filters set — a per-FLW audit-launch
// trigger date). Tab 2 needs actual per-visit-type COMPLETION DATES to check
// "was this done by the trigger date", which the `mothers` SQL pipeline
// (presence-only, evaluated as of "now") cannot answer — see this file's
// module-docstring-equivalent note in mbw_auditing_v6.py. The caller
// (computeTab2ByUser) filters `visitsRows`/`regRows` down to just the FLWs
// being audited THIS run before calling this, so the expensive chronological
// sort + per-mother map-building runs over a handful of FLWs' data, not the
// whole cohort.
function v6_processVisitsFull(visitsRows, taskFilters) {
  ...
  for (var i = 0; i < sorted.length; i++) {
    ...
    var vdt = v6_dateStr(row.visit_datetime);
    var mid = ((row.mother_case_id || '') + '').toLowerCase();
    var formName = v6_normFormName(row.form_name);
    ...
    if (
      ...
      var trigger = v6_dateStr(taskFilters[username] || '');
...
// SLIM per-visit scan for TAB 1 (no task_filters). Attribution, ANC
// eligibility, and per-visit-type completion now come from the `mothers` SQL
// pipeline (see v6_processMothers below) instead of being rebuilt here — this
// loop only computes what genuinely needs a raw visit row: GPS distance,
// visit duration, inter-visit gap (all three need the lag_haversine /
// time-diff values that only exist at visit level — see mbw_auditing_v6.py),
// and which mothers each FLW has EVER visited (`motherSetsByFlw`).
//
// motherSetsByFlw stays visit-level on purpose: it answers "did ANY of this
// FLW's visits touch mother X" (used by num_eligible_mothers_visited below),
// which is NOT the same question as the `mothers` pipeline's
// `attributed_flw` (the LAST FLW to visit — a mother reassigned mid-program
// has exactly one attributed_flw but may appear in more than one FLW's
// motherSetsByFlw).
function v6_processVisitsSlim(visitsRows) {
  var sorted = visitsRows.slice().sort(function (a, b) {
    var ad = a.visit_datetime || '';
    var bd = b.visit_datetime || '';
    return ad < bd ? -1 : ad > bd ? 1 : 0;
...
// Visit-type -> the `mothers` pipeline field(s) that mark it complete.
// Postnatal Delivery Visit carries a second field for the known form-name
// alias ("Post delivery visit" — see V6_FORM_NAME_ALIASES below); coalescing
// two small per-mother counts client-side is cheap, unlike re-deriving it
// from raw visits.
var V6_MOTHERS_FIELD_BY_VISIT_TYPE = {
  'Postnatal Delivery Visit': [
    'postnatal_delivery_completed',
    'postnatal_delivery_completed_alias',
  ],
  '1 Week Visit': ['one_week_completed'],
  '1 Month Visit': ['one_month_completed'],
  '3 Month Visit': ['three_month_completed'],
  '6 Month Visit': ['six_month_completed'],
};

// Turn `mothers` SQL pipeline rows (one per mother) into the same shape of
// lookup maps v6_processVisitsFull used to build by scanning every visit:
// which FLW a mother is attributed to (last visit wins — already computed in
// SQL), whether she has any ANC-ok visit, and which non-ANC visit types are
// complete. Presence-only (as of now) — see mbw_auditing_v6.py for why that's
// sufficient for Tab 1 but not Tab 2.
function v6_processMothers(mothersRows) {
  var attributedFlwByMother = {};
  var ancOkByMother = {};
  var completedByMother = {};
  var mothersByFlw = {};

  for (var i = 0; i < (mothersRows || []).length; i++) {
    var row = mothersRows[i];
    var mid = ((row.mother_case_id || row.entity_id || '') + '').toLowerCase();
    if (!mid) continue;

    var flw = ((row.attributed_flw || '') + '').toLowerCase();
    if (flw) {
      attributedFlwByMother[mid] = flw;
      if (!mothersByFlw[flw]) mothersByFlw[flw] = [];
      mothersByFlw[flw].push(mid);
    }

    ancOkByMother[mid] = (parseInt(row.anc_ok_count || 0, 10) || 0) > 0;

    var completed = {};
    Object.keys(V6_MOTHERS_FIELD_BY_VISIT_TYPE).forEach(function (visitType) {
      var fields = V6_MOTHERS_FIELD_BY_VISIT_TYPE[visitType];
      var any = false;
      for (var f = 0; f < fields.length; f++) {
        if ((parseInt(row[fields[f]] || 0, 10) || 0) > 0) {
          any = true;
          break;
        }
      }
      completed[visitType] = any;
    });
    completedByMother[mid] = completed;
  }

  return {
    attributedFlwByMother: attributedFlwByMother,
    ancOkByMother: ancOkByMother,
    completedByMother: completedByMother,
    mothersByFlw: mothersByFlw,
  };
}

// Process registrations rows. Mirrors v4 handler lines 293-315.
function v6_processRegistrations(regRows) {
  var motherSchedules = {};
...
function v6_processGsForms(gsRows) {
  var gsByUser = {};
...
function v6_computeFollowupRates({
  motherSchedules,
  visitsByMother,
  motherToFlw,
  motherEligibility,
  ...
  for (var mid in motherSchedules) {
    ...
    for (var j = 0; j < schedules.length; j++) {
      ...
      var scheduledStr = v6_dateStr(s.visit_date_scheduled);
      var expiryStr = v6_dateStr(s.visit_expiry_date);
...
// TAB 1 equivalent of v6_computeFollowupRates above: identical grace-period /
// still-eligible arithmetic, but attribution / ANC-ok / completion come from
// `v6_processMothers`'s maps (the SQL `mothers` pipeline) instead of a raw
// visit scan — there is no "as of" date here (Tab 1 always means "as of now"),
// which is exactly the case the `mothers` pipeline's presence-only fields can
// answer.
function v6_computeFollowupRatesFromMothers({
  motherSchedules,
  mothersIndex,
  motherEligibility,
  activeUsernames,
  ...
  for (var mid in motherSchedules) {
    if (!Object.prototype.hasOwnProperty.call(motherSchedules, mid)) continue;
    var schedules = motherSchedules[mid];
    var flw = mothersIndex.attributedFlwByMother[mid];
    if (!flw) continue;
    if (activeSet && !activeSet[flw]) continue;

    var ancOk = !!mothersIndex.ancOkByMother[mid];
    var isEligible = !!(motherEligibility[mid] && ancOk);
    var completed = mothersIndex.completedByMother[mid] || {};
    ...
    for (var j = 0; j < schedules.length; j++) {
      ...
      var expiryStr = v6_dateStr(s.visit_expiry_date);
      var isCompleted =
        visitType === 'ANC Visit' ? ancOk : !!completed[visitType];
      var pastGrace = scheduledStr && scheduledStr <= graceCutoff;
...
// handler lines 401-458. Only called for TAB 2 (task_filters set) — see
// v6_computeMbwAuditingData's branch below.
function v6_computeBaselineFollowupRates({
  motherSchedules,
  visitsByMotherAll,
  motherToFlw,
  taskFilters,
  ...
  Object.keys(taskFilters).forEach(function (flwUsername) {
    var triggerDateStr = v6_dateStr(taskFilters[flwUsername]);
    if (!triggerDateStr) return;
    var triggerGraceCutoff = v6_subtractDays(
      triggerDateStr,
      V6_GRACE_PERIOD_DAYS,
    );
    ...
    baseline[flwUsername] =
      baselineDenominator > 0
        ? v6_round((baselineCompleted / baselineDenominator) * 100)
        : null;
  });
...
function v6_computeMbwAuditingData({
  visitsRows,
  visitsAggRows,
  mothersRows,
  regRows,
  ...
  var today = v6_todayStr(currentDate);
  var graceCutoff = v6_subtractDays(today, V6_GRACE_PERIOD_DAYS);

  var r = v6_processRegistrations(regRows || []);
  var gsByUser = v6_processGsForms(gsRows || []);

  // Two branches produce the same shape of lookup maps, so the FLW-summary
  // loop below is IDENTICAL either way:
  //   motherToFlwMap        mid -> attributed FLW (lowercased)
  //   ancOkMap               mid -> has an ANC-ok visit
  //   completedByMotherMap    mid -> {visitType: truthy}
  //   gpsDistances/visitDurations/interVisitGaps/motherSetsByFlw   per FLW
  var motherToFlwMap, ancOkMap, completedByMotherMap;
  var gpsDistances, visitDurations, interVisitGaps, motherSetsByFlw;
  var bfCountByFlw = {},
    ebfCountByFlw = {},
    visitsCompletedByFlw = {};
  var useAggCounts = false;
  var numMothersByFlw = {};
  var flwFuResult, baselineRates;

  if (taskFilters) {
    // TAB 2: baseline follow-up rate as of each flagged FLW's own audit-launch
    // date. The `mothers` pipeline is presence-as-of-now only (see
    // mbw_auditing_v6.py), so an arbitrary historical trigger date needs the
    // full chronological scan — same algorithm v5 always used. The caller
    // (computeTab2ByUser) pre-filters visitsRows/regRows down to just the
    // FLWs being audited this run, so this runs over a handful of FLWs'
    // mothers, not the cohort.
    var v = v6_processVisitsFull(visitsRows || [], taskFilters);
    motherToFlwMap = v.motherToFlw;
    ancOkMap = v.ancOkMothers;
    completedByMotherMap = v.visitsByMother;
    gpsDistances = v.gpsDistances;
    visitDurations = v.visitDurations;
    interVisitGaps = v.interVisitGaps;
    motherSetsByFlw = v.motherSetsByFlw;
    bfCountByFlw = v.bfCountByFlw;
    ebfCountByFlw = v.ebfCountByFlw;
    visitsCompletedByFlw = v.visitsCompletedByFlw;

    flwFuResult = v6_computeFollowupRates({
    ...
    baselineRates = v6_computeBaselineFollowupRates({
      motherSchedules: r.motherSchedules,
    ...
    // TAB 1: attribution / ANC-ok / per-visit-type completion come from the
    // `mothers` SQL pipeline (one row per mother, computed once and cached)
    // instead of a chronological scan over every visit. GPS/duration/gap
    // still need a visit-level pass (see v6_processVisitsSlim's docstring for
    // why), but that pass no longer builds attribution or eligibility maps.
    var slim = v6_processVisitsSlim(visitsRows || []);
    var mi = v6_processMothers(mothersRows || []);
    motherToFlwMap = mi.attributedFlwByMother;
    ancOkMap = mi.ancOkByMother;
    completedByMotherMap = mi.completedByMother;
    gpsDistances = slim.gpsDistances;
    visitDurations = slim.visitDurations;
    interVisitGaps = slim.interVisitGaps;
    motherSetsByFlw = slim.motherSetsByFlw;
    baselineRates = {};

    // SQL-aggregated counts for num_mothers/bf_count/ebf_count/visits_completed
    // instead of a per-visit-scan tally. Matches v5's `use_agg_counts` branch,
    // now unconditional for Tab 1 since visits_completed moved into SQL too.
    useAggCounts = !!(visitsAggRows && visitsAggRows.length);
    if (useAggCounts) {
      for (var i = 0; i < visitsAggRows.length; i++) {
        ...
        if (!isNaN(bc)) bfCountByFlw[u] = bc;
        var ec = parseInt(aggRow.ebf_count || 0, 10);
        if (!isNaN(ec)) ebfCountByFlw[u] = ec;
        var vc = parseInt(aggRow.visits_completed || 0, 10);
        if (!isNaN(vc)) visitsCompletedByFlw[u] = vc;
      }
    }

    flwFuResult = v6_computeFollowupRatesFromMothers({
      motherSchedules: r.motherSchedules,
      mothersIndex: mi,
      motherEligibility: r.motherEligibility,
      activeUsernames: activeUsernames || [],
  ...
  // Build FLW summaries. Match v4 line 475-561 exactly (shape/logic unchanged
  // from v5 from here down — only where the input maps came from differs).
  ...
  if (activeSet.length > 0) {
    ...
    var set = {};
    Object.values(motherToFlwMap).forEach(function (u) {
      if (u) set[u] = true;
    });
  ...
  for (var n = 0; n < targetSet.length; n++) {
    ...
    var fu = flwFuResult[u] || {};
    var dists = gpsDistances[u] || [];

    var mothersVisited = motherSetsByFlw[u] || {};
    ...
    // Uses motherToFlwMap for last-visit-wins attribution, consistent with total_eligible.
    Object.keys(motherToFlwMap).forEach(function (mid3) {
      if (motherToFlwMap[mid3] !== u) return;
      if (!(r.motherEligibility[mid3] && ancOkMap[mid3])) return;
      var cnt = 1; // ANC confirmed (eligibility gate)
      var mv = completedByMotherMap[mid3] || {};
      for (var vti = 0; vti < V6_NON_ANC_VISIT_TYPES.length; vti++) {
        if (mv[V6_NON_ANC_VISIT_TYPES[vti]]) cnt++;
      }
      if (cnt >= 5) fivePlusMothers++;
    });
    var visitsCompleted = visitsCompletedByFlw[u] || 0;

    var bfCount = bfCountByFlw[u] || 0;
    var ebfCount = ebfCountByFlw[u] || 0;
    var ebfPct = bfCount > 0 ? v6_round((ebfCount / bfCount) * 100) : null;

    var denom = fu.filtered_denominator || 0;
    var completedFu = fu.filtered_completed || 0;
    var followupRate = denom > 0 ? v6_round((completedFu / denom) * 100) : null;

    var stillElig = fu.still_eligible || 0;
    var pctStillEligible =
      totalEligible > 0 ? v6_round((stillElig / totalEligible) * 100) : null;
    ...
    if (dists.length > 0) {
      ...
      revisitM = v6_round(meanM);
      meterPerVisit = v6_round(medianM);
      // dist_ratio is round(meanM/medianM, 2) in v4. Banker's rounding at
      // the 2-decimal grain matches what Python does to floats.
      distRatio = medianM > 0 ? v6_round((meanM / medianM) * 100) / 100 : null;
    }

    var gsRaw = gsByUser[u];
    var gsScore = gsRaw != null ? v6_round(gsRaw) : null;

    var durations = visitDurations[u] || [];
    var minutePerVisit = null;
    if (durations.length > 0) {
      var sortedDur = durations.slice().sort(function (a, b) {
        return a - b;
      });
      minutePerVisit = v6_round(sortedDur[Math.floor(sortedDur.length / 2)]);
    }

    var gaps = interVisitGaps[u] || [];
    var travelTime = null;
    if (gaps.length > 0) {
      var sortedGaps = gaps.slice().sort(function (a, b) {
        return a - b;
      });
      travelTime = v6_round(sortedGaps[Math.floor(sortedGaps.length / 2)]);
    }
...
function WorkflowUI({
  ...
  // flw_summaries via v6_computeMbwAuditingData — line-for-line port of v4's
  // Python handler.
  var runAnalysis = React.useCallback(
    function () {
      ...
      if (isCompleted) {
        ...
        var snapResult = v6_computeMbwAuditingData({
      ...
      var mothersRows =
        (srcPipelines.mothers && srcPipelines.mothers.rows) || [];
      var regRows =
      ...
      var computeResult;
      try {
        computeResult = v6_computeMbwAuditingData({
          visitsRows: visitsRows,
          visitsAggRows: visitsAggRows,
          mothersRows: mothersRows,
          regRows: regRows,
          gsRows: gsRows,
  ...
  // Tab 2 job runner
  // =========================================================================
  // v5: Tab 2 baseline-rate compute. Re-runs the same v6_computeMbwAuditingData
  ...
  var computeTab2ByUser = function (
    ...
    // Pre-filter to just the FLWs being audited THIS run, before the full
    // chronological scan (v6_processVisitsFull) runs at all. v5 always ran
    // that scan over the whole cohort's visits/registrations even though Tab
    // 2 only ever grades the handful of flagged FLWs; this is what actually
    // shrinks Tab 2's cost, since the (presence-only, as-of-now) `mothers`
    // pipeline can't answer Tab 2's per-FLW trigger-date question — see
    // v6_computeMbwAuditingData's task_filters branch.
    var flaggedSet = {};
    flaggedUsernames.forEach(function (u) {
      flaggedSet[((u || '') + '').toLowerCase()] = true;
    });
    ...
    // registrations is already one row per MOTHER (not per visit), so this is
    // a smaller win than the visits filter above — but it still skips
    // indexing every other FLW's mothers before v6_computeBaselineFollowupRates
    // narrows to these usernames anyway. Same mother_case_id resolution
    // v6_processRegistrations uses (schedules[].mother_case_id, falling back
    // to row.mother_case_id).
    var flaggedMotherIds = {};
    visitsRows2.forEach(function (row) {
      var mid = ((row.mother_case_id || '') + '').toLowerCase();
      if (mid) flaggedMotherIds[mid] = true;
    });
    var regRows2 = (
      ...
      for (var j = 0; j < schedules.length; j++) {
        var s = schedules[j];
        if (s && typeof s === 'object' && s.mother_case_id) {
          mid = ((s.mother_case_id || '') + '').toLowerCase();
          break;
        }
      }
      if (!mid) mid = ((row.mother_case_id || '') + '').toLowerCase();
      return !!mid && !!flaggedMotherIds[mid];
    });
    var gsRows2 =
      (srcPipelinesArg.gs_forms && srcPipelinesArg.gs_forms.rows) || [];

    var tab2Result = v6_computeMbwAuditingData({
  ...
  // =========================================================================
  // Monthly snapshot metrics — used by the improvement-over-time chart.
  // Filters visits to those submitted on or before snapDateStr (YYYY-MM-DD),
  // then replicates the core follow-up-rate and still-eligible computation
  // from v6_computeFollowupRates using that cutoff as "today".
  // =========================================================================
  ...
  var computeMonthlySnapshot = function (
    visitsRows,
    regRows,
    snapDateStr,
    activeUsernames,
  ) {
    // Sort chronologically so last-visit-wins attribution matches v6_processVisits.
    ...
    // v6_processVisits behaviour where attribution = last-visit-wins globally.
    var motherToFlw = {};
    ...
    if (activeUsernames && activeUsernames.length) {
      activeSet = {};
      for (var k = 0; k < activeUsernames.length; k++)
        activeSet[activeUsernames[k].toLowerCase()] = true;
    }
    ...
    for (var i = 0; i < visitsRows.length; i++) {
      ...
      if (
        !row.visit_datetime ||
        row.visit_datetime.substring(0, 10) > snapDateStr
      )
        continue;
      var mid = (row.mother_case_id || '').toLowerCase();
      if (!mid) continue;
      ...
      if (formName) {
        if (!visitsByMother[mid]) visitsByMother[mid] = {};
        if (!visitsByMother[mid][formName])
          visitsByMother[mid][formName] = row.visit_datetime;
      }
    }

    var r = v6_processRegistrations(regRows);
    var graceCutoff = v6_subtractDays(snapDateStr, V6_GRACE_PERIOD_DAYS);
    ...
    for (var mid2 in r.motherSchedules) {
      if (!Object.prototype.hasOwnProperty.call(r.motherSchedules, mid2))
        continue;
      if (activeSet) {
      ...
      for (var j = 0; j < schedules.length; j++) {
        ...
        if (pastGrace) {
          totDenom++;
          if (isComp) totCompleted++;
        }
      }

      if (isEligible) {
        totEligible++;
        if (missedCount < 2) totStillEligible++;
      }
    }

    return {
      followup_rate:
        totDenom > 0 ? Math.round((totCompleted / totDenom) * 100) : null,
      pct_still_eligible:
        totEligible > 0
          ? Math.round((totStillEligible / totEligible) * 100)
          : null,
    };
  };
