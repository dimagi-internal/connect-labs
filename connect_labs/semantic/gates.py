...
Credibility is not decided here. It is the compiler's `suppression` rules and
`snapshot.grade`'s spec mapping (`credibility: {<indicator>: <setting>}`) onto the
deployment's settings -- both data. A `credible_for` helper that named C14, C18
and C22 in code lived here, unused outside its own tests, and was removed when
the engine stopped knowing which registry it was grading.
"""
