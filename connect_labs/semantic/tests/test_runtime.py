...
def test_the_one_family_carries_every_measure_the_registry_declares():
    """KMC is one set now (#2004): asking for its family returns all of it, and the
    family name is case-insensitive the way a query-string param arrives."""
    _, inds = load_registry("kmc")
    assert {m["name"] for m in filter_to_series(inds, "kmc")["measures"]} == {m["name"] for m in inds["measures"]}
...
def test_the_registry_actually_runs_and_returns_rows(fixture_visits):
    ...
    assert "started_cases" in row and "started_cases_denominator" in row
...
def test_a_failing_query_raises_a_readable_error_not_a_raw_driver_traceback(db):
    ...
    with pytest.raises(SemanticRuntimeError, match="semantic query failed"):
        with transaction.atomic():
            evaluate(None, [10042], visit_sql="SELECT * FROM no_such_relation", registry_name="kmc", series="KMC")
...
def test_selecting_a_series_does_not_drag_in_the_other_series_parts():
    ...
    KMC has one family now, so this is pinned on a two-family registry.
    """

    def ind(name):
        return [
            {"name": name, "type": "number", "sql": "{" + name + "_numerator}", "meta": {"indicator": name.upper()}},
            {"name": f"{name}_numerator", "type": "count"},
            {"name": f"{name}_denominator", "type": "count"},
        ]

    two = {"series": ["A", "B"], "measures": ind("a01") + ind("b01")}
    a_names = {m["name"] for m in filter_to_series(two, "A")["measures"]}
    assert a_names == {"a01", "a01_numerator", "a01_denominator"}, "B's parts leaked into A"
    b_names = {m["name"] for m in filter_to_series(two, "B")["measures"]}
    assert b_names == {"b01", "b01_numerator", "b01_denominator"}, "A's parts leaked into B"
...
def test_an_indicator_keeps_its_denominator_even_when_its_value_never_references_it():
    ...
    _, inds = load_registry("kmc")
    names = {m["name"] for m in filter_to_series(inds, "KMC")["measures"]}
    for ind in ("total_cases", "registered_cases", "started_cases", "cumulative_svns_reached"):
...
def test_mortality_is_two_sided_because_implausibly_low_means_under_recording():
    ...
    _, inds = load_registry("kmc")
    mort = [c for c in measure_catalog(inds) if c["indicator"] == "mortality"][0]
    assert mort["direction"] == "mid2"
    assert mort["bands"][0][0] == 4 and mort["bands"][1][0] == 2


def test_percent_bands_are_on_the_percent_scale_the_measures_return():
    """The % measures return 100.0 * num/den. The workbook stored its thresholds as
    FRACTIONS, so a threshold copied across without rescaling would be wrong by 100x
    and would band everything green."""
    from connect_labs.semantic.runtime import measure_catalog

    _, inds = load_registry("kmc")
    for c in measure_catalog(inds):
...
def test_evaluate_refuses_the_raw_schema_dict_by_name():
    ...
    with pytest.raises(SemanticRuntimeError, match="AnalysisPipelineConfig"):
        evaluate({"fields": [], "terminal_stage": "entity"}, [10042], registry_name="kmc", series="KMC")
...
def test_the_case_scope_returns_one_row_per_baby_and_a_worker_filter_narrows_it(fixture_visits):
    ...
    for r in rows:
        assert r["started_cases_denominator"] == 1
        assert r["started_cases"] in (0.0, 1.0)
    one = evaluate(
        None,
        [10042],
        visit_sql=fixture_visits,
