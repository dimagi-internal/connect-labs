...
YAML flow mappings (`meta: { ... }`) split on bare commas. Mortality's `scope_note`
...
def test_mortality_scope_note_is_the_whole_caveat(measures):
    mortality = next(m for m in measures if m["name"] == "mortality")
    assert "non-recorders add denominator without deaths" in mortality["meta"]["scope_note"]
