...
(function () {
  ...
  // --- the indicator menu -------------------------------------------------
  //
  // Fifty-two indicators in a <select> is a scroll bar with a catalogue hidden
  // inside it. The point of this menu is that the catalogue is VISIBLE: every
  // group at once, in columns, so a reader discovers the system can answer a
  // question they had not thought to ask. Grouping is registry data
  // (measures.GROUPS), not a shape invented here.

  var menuOpen = false;

  // The old <select> populated indicatorMeta as a side effect of building its
  // <option>s, and applyThresholdScale reads it. Made explicit rather than
  // left to fall out of rendering, because the menu renders lazily on open and
  // the slider scale is needed before anyone opens it.
  ...
  function indicatorsByGroup() {
    var groups = methodInfo.groups || [];
    var bucket = {};
    (methodInfo.indicators || []).forEach(function (i) {
      var g = i.group || 'Other';
      (bucket[g] = bucket[g] || []).push(i);
    });
    // Registry order, then anything the registry did not place — which should
    // be nothing, and is visible rather than silently dropped if it is not.
  ...
  function renderIndicatorMenu(filter) {
    ...
    indicatorsByGroup().forEach(function (group) {
      ...
      items.forEach(function (i) {
        ...
        b.setAttribute(
          'aria-selected',
          i.code === currentIndicator ? 'true' : 'false',
        );
        b.title = i.description || i.label;

        var mark = document.createElement('span');
        // Which way the threshold points. A burden selects above it, a
        // coverage measure below — opposite questions, and the one thing a
        // reader must know before touching the slider.
        mark.className =
          'tg-opt-mark ' +
  ...
  function renderIndicatorTrigger() {
    ...
    if (label) label.textContent = meta.label || currentIndicator;
  ...
  // The ONE way the indicator changes. Three callers used to reach for the
  // <select>'s .value and .onchange — the URL restore, the costing preset, and
  // the menu itself — so the behaviour lived in an event handler and every
  // caller had to remember to fire it.
  function selectIndicator(code) {
    if (!code || code === currentIndicator) return Promise.resolve();
    ...
    return fetch(
      TG.urls.methods + '?indicator=' + encodeURIComponent(currentIndicator),
    )
      .then(function (r) {
        return r.json();
      })
      .then(function (info) {
        ...
        renderPicker();
        renderIndicatorTrigger();
        // Measurement depth is per indicator: a different indicator can have a
        // different deepest measured level, and the level buttons have to say
        // so before they are clicked.
        return refreshScope().then(reload);
      });
  }
  ...
  function initIndicatorMenu() {
    ...
    trigger.addEventListener('click', function (ev) {
      ev.stopPropagation();
      if (menuOpen) closeIndicatorMenu();
      else openIndicatorMenu();
    });
    if (search) {
      search.addEventListener('input', function () {
        renderIndicatorMenu(search.value);
      });
  ...
  function renderCostControls() {
    ...
    presetSel.onchange = function () {
      ...
      if (pick.targets && pick.targets !== currentIndicator) {
        return selectIndicator(pick.targets);
      }
