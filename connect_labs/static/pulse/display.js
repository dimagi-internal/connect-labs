...
(function () {
  ...
  function initBasemap() {
    ...
    m.on('load', () => {
      // Dimming the basemap so the geography stops competing with the data.
      // Shared with the opportunity map and the network page — one behaviour in
      // one place. See ConnectMap.calmBasemap.
      window.ConnectMap.calmBasemap(m);
      baseDirty = true;
      setFocus(focus, true);
    });
    return m;
