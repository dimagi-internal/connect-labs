"""Program tags: Connect's vocabulary, not one this app invented.
...
class TestTheVocabulary:
    def test_a_known_slug_reads_as_connects_name_for_it(self):
        assert programs.label("kmc") == "Kangaroo Mother Care"
        assert programs.label("chc") == "Child Health Campaign"
    ...
    def test_an_unknown_slug_stays_visibly_a_code(self):
        ...
        assert programs.label("zzz") == "ZZZ"

    def test_the_unclassified_bucket_is_not_a_program(self):
        """Connect's `other` means "no delivery type recorded". Treating it as a
        program would put 264 opportunities behind a label meaning "unknown".
        """
        assert not programs.is_program("other")
        assert programs.chips(["other", "kmc"]) == [{"slug": "kmc", "label": "Kangaroo Mother Care"}]

    def test_a_round_that_is_not_a_program_says_so_rather_than_going_blank(self):
        """A matching grant is a funding instrument. "Nobody has tagged this"
        and "somebody looked and it is not a program" are different states,
        and only the first is work outstanding."""
        assert programs.label(programs.PROGRAM_NONE) == "Cross-program"
        assert not programs.is_program(programs.PROGRAM_NONE)
        assert programs.chips([programs.PROGRAM_NONE]) == []

    def test_chips_deduplicate_and_sort_by_what_is_read(self):
        assert [c["label"] for c in programs.chips(["readers", "kmc", "readers"])] == [
...
class TestDeliveredComesFromPulseNotFromAForm:
    def test_an_organisation_carries_the_programs_its_opportunities_ran(self, one_org):
        assert queries.delivered_programs_by_org_name()["Lakeside Health Trust"] == {"kmc"}

    def test_the_unclassified_opportunity_does_not_become_a_program(self, one_org):
        """`delivers=["kmc", "other"]` seeded two opportunities; only one names
        a program."""
        assert "other" not in queries.delivered_programs_by_org_name()["Lakeside Health Trust"]

    def test_delivered_and_applied_do_not_bleed_into_each_other(self, one_org):
        org = queries.all_rows_with_rounds()[0]
        assert queries.delivered_programs_by_org_name()["Lakeside Health Trust"] == {"kmc"}
        assert queries.applied_programs_of(org) == {"nutrition"}


@pytest.mark.django_db
class TestThePagesShowTheTags:
    def test_the_round_page_names_its_program(self, client, user, one_org):
        client.force_login(user)
        body = client.get(reverse("marketplace:round", args=["rutf-2026"])).content.decode()
        assert "Ready-to-Use Therapeutic Food (RUTF)" in body

    def test_an_untagged_round_shows_no_program_rather_than_a_wrong_one(self, client, user, one_org):
        ...
        assert response.context["round"].program_label == ""
    ...
    def test_the_organisation_page_separates_delivered_from_applied(self, client, user, one_org):
        ...
        assert [p["label"] for p in context["delivered_programs"]] == ["Kangaroo Mother Care"]
        assert [p["label"] for p in context["applied_programs"]] == ["Ready-to-Use Therapeutic Food (RUTF)"]
...
class TestWaterIsAKnownProgram:
    """Chlorine dispensers needed a program and Connect had none that fitted.
    ...
    def test_water_reads_as_a_name_not_a_code(self):
        assert programs.label("water") == "Water"

    def test_water_is_a_real_program_so_it_can_be_filtered_on(self):
        assert programs.is_program("water")
        assert programs.chips(["water"]) == [{"slug": "water", "label": "Water"}]
    ...
    def test_a_new_type_with_no_opportunities_yet_still_works_on_the_applied_side(self, db):
        ...
        assert queries.applied_programs_of(fetched) == {"water"}
        assert queries.delivered_programs_by_org_name().get("Riverbank Water Trust") is None


class TestEveryLiveProgramHasAName:
    ...
    def test_the_last_three_unnamed_types_are_named(self):
        assert programs.label("tms") == "Turmeric Market Survey"
        assert programs.label("conversation") == "Low-resource languages"
        assert programs.label("cholera") == "Cholera"
    ...
    def test_every_delivery_type_seen_in_production_has_a_name(self):
        ...
        # comparing against `.upper()` calls a named program unnamed.
        unnamed = live - programs.known_slugs()
