...
_REAL_REQUIRE_OPPORTUNITY_ACCESS = connect_labs.mcp.tools.synthetic._require_opportunity_access
...
def test_synthetic_generate_from_manifest_creates_folder_and_row(user, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda opp_id, user: __import__(
            "connect_labs.labs.synthetic.generator.fixtures.schema_loader",
            fromlist=["FormSchema"],
...
def test_all_phase6_tools_are_registered():
    ...
    from connect_labs.mcp.tool_registry import list_tools
...
def test_accessible_opp_ids_uses_user_token(user, monkeypatch):
    ...
    import connect_labs.labs.integrations.connect.oauth as oauth_mod
