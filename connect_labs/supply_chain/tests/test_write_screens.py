...
from connect_labs.supply_chain.models import Commodity, Outreach, Quote, Supplier, Tender
...
class TestCreatingATender:
    def test_the_form_renders_with_its_commodity_lines(self, scoped, rutf):
        response = scoped.get(reverse("supply_chain:procurement_tender_create"))
        assert response.status_code == 200
        body = response.content.decode()
        assert "What this tender is asking for" in body
    ...
    def test_a_new_tender_opens_on_exactly_one_blank_commodity_row(self, scoped, rutf):
        ...
        """
        body = scoped.get(reverse("supply_chain:procurement_tender_create")).content.decode()
        rows = set(re.findall(r'name="lines-(\d+)-commodity_slug"', body))
        assert rows == {"0"}, f"one blank row, got {sorted(rows)}"

    def test_a_tender_is_created_with_its_lines_and_delivery_point(self, scoped, rutf):
        response = scoped.post(
            reverse("supply_chain:procurement_tender_create"),
            {
                "label": "Tender 1 — RUTF",
                "places-TOTAL_FORMS": "1",
                "places-INITIAL_FORMS": "0",
                "places-MIN_NUM_FORMS": "0",
                "places-MAX_NUM_FORMS": "1000",
                "places-0-name": "Central store",
                "places-0-city": "Kano",
                "places-0-country_name": "Nigeria",
        ...
        created = Tender.objects.get(label="Tender 1 — RUTF")
        assert response.status_code == 302
        assert response.url == reverse("supply_chain:procurement_tender_detail", args=[created.pk])
        assert created.program_id == PROGRAM
        assert created.status == "draft", "a new tender opens in draft, not open"
    ...
    def test_a_quantity_survives_as_an_exact_string_not_a_float(self, scoped, rutf):
        ...
        tenders them. A form layer that parsed them into floats would undo that
        ...
        stored = Tender.objects.get(label="Precision").lines[0]["quantity"]
        assert stored == "1234.5678"
        assert Decimal(stored) == Decimal("1234.5678")

    def test_a_tender_with_no_commodity_is_refused_and_keeps_what_was_typed(self, scoped, rutf):
        response = scoped.post(
            reverse("supply_chain:procurement_tender_create"),
            {
                "label": "Empty tender",
        ...
        assert not Tender.objects.filter(label="Empty tender").exists()
        assert "Empty tender" in response.content.decode(), "what was typed must survive the refusal"


class TestOpeningAndClosing:
    def test_open_is_a_post_only_action(self, scoped, a_tender):
        assert scoped.get(reverse("supply_chain:procurement_tender_open", args=[a_tender.pk])).status_code == 405

    def test_opening_a_tender_lets_it_take_quotes(self, scoped, a_tender):
        response = scoped.post(reverse("supply_chain:procurement_tender_open", args=[a_tender.pk]))
        a_tender.refresh_from_db()
        assert response.status_code == 302
        assert a_tender.status == "open"

    def test_a_tender_with_no_delivery_point_is_refused_on_its_own_page(self, scoped, rutf):
        """The operation's refusal is an instruction, so it belongs back where
        the button was rather than on an error page."""
        naked = Tender.objects.create(program_id=PROGRAM, label="No delivery point", lines=[], delivery_point={})
        response = scoped.post(reverse("supply_chain:procurement_tender_open", args=[naked.pk]), follow=True)
        naked.refresh_from_db()
        assert naked.status == "draft"
        assert any("delivery" in str(m).lower() for m in response.context["messages"])

    def test_closing_a_tender_stops_further_quotes(self, scoped, a_tender):
        a_tender.status = "open"
        a_tender.save()
        scoped.post(reverse("supply_chain:procurement_tender_close", args=[a_tender.pk]))
        a_tender.refresh_from_db()
        assert a_tender.status == "closed"


class TestInvitations:
    def test_the_supplier_picker_offers_only_this_scope(self, scoped, a_tender, supplier):
        ...
        body = scoped.get(reverse("supply_chain:procurement_outreach_log", args=[a_tender.pk])).content.decode()
        assert "Northwind Foods" in body
        assert "Somebody Else Ltd" not in body

    def test_recording_an_invitation(self, scoped, a_tender, supplier):
        response = scoped.post(
            reverse("supply_chain:procurement_outreach_log", args=[a_tender.pk]),
            {"supplier": str(supplier.pk), "channel": "manual", "sent_on": "2026-04-28", "notes": "emailed sales@"},
        )
        logged = Outreach.objects.get(tender=a_tender, supplier=supplier)
        assert response.status_code == 302
        assert logged.sent_on == date(2026, 4, 28)
        assert logged.responded is False

    def test_a_field_the_form_does_not_declare_cannot_be_smuggled_in(self, scoped, a_tender, supplier):
        """`tender_id` comes from the URL and is not a form field, so Django
        ...
        """
        other = Tender.objects.create(program_id=PROGRAM, label="Other", lines=[], delivery_point={"name": "x"})
        scoped.post(
            reverse("supply_chain:procurement_outreach_log", args=[a_tender.pk]),
            {"supplier": str(supplier.pk), "channel": "manual", "tender_id": str(other.pk)},
        )
        assert Outreach.objects.filter(tender=a_tender).count() == 1
        assert not Outreach.objects.filter(tender=other).exists()

    def test_recording_a_reply(self, scoped, a_tender, supplier):
        invitation = Outreach.objects.create(tender=a_tender, supplier=supplier, sent_on=date(2026, 4, 28))
        scoped.post(
    ...
    def test_deleting_an_invitation_needs_a_reason(self, scoped, a_tender, supplier):
        invitation = Outreach.objects.create(tender=a_tender, supplier=supplier)
        refused = scoped.post(
...
class TestVoidingAQuote:
    @pytest.fixture
    def quote(self, a_tender, rutf, supplier):
...
class TestTheScreensRefuseWithoutAProgramme:
    def test_every_write_screen_asks_for_a_programme_rather_than_raising(self, client, user, a_tender):
        ...
        for name, args in (
            ("procurement_tender_create", []),
            ("procurement_outreach_log", [a_tender.pk]),
...
class TestTheScreensAreReachable:
    ...
    def test_a_searchable_picker_actually_loads_the_library(self, scoped, a_tender, supplier):
    ...
    def test_the_board_offers_a_new_tender(self, scoped):
        body = scoped.get(reverse("supply_chain:procurement_tender_board")).content.decode()
        assert reverse("supply_chain:procurement_tender_create") in body

    def test_a_draft_tender_offers_opening_and_an_open_one_offers_closing(self, scoped, a_tender):
        draft = scoped.get(reverse("supply_chain:procurement_tender_detail", args=[a_tender.pk])).content.decode()
        assert reverse("supply_chain:procurement_tender_open", args=[a_tender.pk]) in draft
        assert reverse("supply_chain:procurement_tender_close", args=[a_tender.pk]) not in draft

        a_tender.status = "open"
        a_tender.save()
        opened = scoped.get(reverse("supply_chain:procurement_tender_detail", args=[a_tender.pk])).content.decode()
        assert reverse("supply_chain:procurement_tender_close", args=[a_tender.pk]) in opened
        assert reverse("supply_chain:procurement_tender_open", args=[a_tender.pk]) not in opened

    def test_opening_and_closing_are_posts_not_links(self, scoped, a_tender):
        """A link that mutates is one prefetch away from doing it unasked."""
        body = scoped.get(reverse("supply_chain:procurement_tender_detail", args=[a_tender.pk])).content.decode()
        open_url = reverse("supply_chain:procurement_tender_open", args=[a_tender.pk])
        assert f'action="{open_url}"' in body, "must be a form action"
        assert f'href="{open_url}"' not in body, "must not be a link"

    def test_an_invitation_row_offers_reply_and_delete(self, scoped, a_tender, supplier):
        invitation = Outreach.objects.create(tender=a_tender, supplier=supplier, sent_on=date(2026, 4, 28))
        body = scoped.get(reverse("supply_chain:procurement_tender_detail", args=[a_tender.pk])).content.decode()
        assert reverse("supply_chain:procurement_outreach_log", args=[a_tender.pk]) in body
    ...
    def test_a_standing_quote_offers_voiding_and_a_voided_one_does_not(self, scoped, a_tender, rutf, supplier):
...
class TestRecordingAQuote:
    ...
    def test_a_quote_is_recorded_as_stated(self, scoped, a_tender, supplier, rutf):
        response = scoped.post(
            reverse("supply_chain:procurement_quote_entry"),
            self._post(tender=a_tender.pk, supplier=supplier.pk, commodity=rutf.pk),
        )
        assert response.status_code == 302

        made = Quote.objects.get(tender=a_tender, supplier=supplier)
    ...
    def test_the_pickers_offer_only_this_programme(self, scoped, a_tender, supplier, rutf):
        ...
        Tender.objects.create(program_id=99999, label="A tender in another programme", lines=[], delivery_point={})
        ...
        for foreign in (
            ...
            "A tender in another programme",
        ):
            assert foreign not in body, foreign

    def test_a_price_with_no_basis_is_refused(self, scoped, a_tender, supplier, rutf):
        """Without it a price compares with nothing, and comparing is what the
        whole tender is for."""
        response = scoped.post(
            reverse("supply_chain:procurement_quote_entry"),
            self._post(tender=a_tender.pk, supplier=supplier.pk, commodity=rutf.pk, as_quoted_unit=""),
        )
        assert response.status_code == 200
        assert "as_quoted_unit" in response.context["form"].errors
        assert not Quote.objects.filter(tender=a_tender).exists()

    def test_a_trade_item_that_is_not_a_version_of_the_product_is_refused(self, scoped, a_tender, supplier, rutf):
        ...
        response = scoped.post(
            reverse("supply_chain:procurement_quote_entry"),
            self._post(tender=a_tender.pk, supplier=supplier.pk, commodity=rutf.pk, item=wrong.pk),
        )
        assert response.status_code == 200
        assert "item" in response.context["form"].errors

    def test_arriving_from_a_tender_preselects_it(self, scoped, a_tender):
        body = scoped.get(f"{reverse('supply_chain:procurement_quote_entry')}?tender={a_tender.pk}").content.decode()
        chosen = re.search(r'<option value="(\d+)"\s+selected', body)
        assert chosen and chosen.group(1) == str(a_tender.pk)

    def test_a_nonsense_tender_parameter_does_not_raise(self, scoped):
        """A URL is somebody else's input, including a bookmarked one."""
        assert scoped.get(f"{reverse('supply_chain:procurement_quote_entry')}?tender=notanumber").status_code == 200

    def test_the_price_crosses_as_an_exact_string_and_the_product_by_slug(self, scoped, a_tender, supplier, rutf):
        """Invisible to a tender-trip: 52.42 tenders back through a float, and a
...
class TestThePayloadBoundary:
    ...
    def test_what_the_url_fixes_beats_what_the_form_collected(self):
        ...
        view.kwargs = {}
        view.fixed = lambda **kw: {"data": {"tender_id": 999}}

        class _Form:
            def payload(self):
                return {"tender_id": 1, "supplier_id": 7}

        captured = {}
        view.op = lambda name, **payload: captured.update(payload) or {"tender_id": 999}
        view.redirect_to = lambda result: "/"
        assert get_operation("outreach_log")  # the operation really exists
        view.form_valid(_Form())
        assert captured["data"]["tender_id"] == 999, "the URL's value must win over a posted one"
