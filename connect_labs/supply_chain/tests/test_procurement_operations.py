...
def _access_for(tender, commodity, quotes, suppliers=(), quote_by_id=None):
    access = MagicMock()
    access.get_tender.return_value = tender
...
def test_quote_get_reports_the_audience_on_each_missing_fact(rutf, tender_2000_cartons):
    q = quote(pack_spec_source="not_stated", base_per_pack_stated=None)
    access = MagicMock()
    access.get_quote.return_value = q
    access.get_tender.return_value = tender_2000_cartons
...
def test_award_create_rejects_a_nonexistent_quote():
    ...
    with pytest.raises(ValueError, match="not found"):
        award_create(access, tender_id=1, quote_id=999, rationale="cheapest defensible option")

    assert not access.create_award.called


def test_award_create_rejects_a_quote_from_a_different_tender(rutf, tender_2000_cartons):
    """A quote quoted for tender 2 must not be awardable against tender 1 --
    the frozen comparison_snapshot is built from tender 1's own comparison
    and would never contain the quote it claims to have chosen."""
    q = quote(tender_id=2)
    access = _access_for(tender_2000_cartons, rutf, [q], quote_by_id={q.id: q})

    with pytest.raises(ValueError, match="belongs to tender 2"):
        award_create(access, tender_id=1, quote_id=q.id, rationale="cheapest defensible option")

    assert not access.create_award.called


def test_award_create_rejects_a_voided_quote(rutf, tender_2000_cartons):
    q = quote(tender_id=1, voided=True, void_reason="duplicate")
    access = _access_for(tender_2000_cartons, rutf, [q], quote_by_id={q.id: q})
...
def test_award_create_rejects_a_superseded_quote(rutf, tender_2000_cartons):
    q = quote(tender_id=1, superseded_by_quote_id=99)
    access = _access_for(tender_2000_cartons, rutf, [q], quote_by_id={q.id: q})
...
def test_award_create_freezes_a_snapshot_a_later_quote_does_not_retroactively_change(rutf, tender_2000_cartons):
    ...
    tender/commodity AFTER the award must not retroactively alter the
    snapshot object already handed to access.create_award."""
    winner = quote(tender_id=1, supplier_id=1, as_quoted_amount="50.00")
    northwind = wrap(Supplier, {"name": "Northwind Nutrition"}, record_id=1)
    access = _access_for(tender_2000_cartons, rutf, [winner], suppliers=[northwind], quote_by_id={winner.id: winner})
    ...
    award_create(access, tender_id=1, quote_id=winner.id, rationale="cheapest defensible option")
    ...
    # A cheaper quote lands on the same tender/commodity after the award --
    # a FRESH compare_tender call now ranks it first.
    later = quote(tender_id=1, supplier_id=2, as_quoted_amount="1.00")
    fresh = compare_tender(tender_2000_cartons, rutf, [winner, later], {1: northwind})
