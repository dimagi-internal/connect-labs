...
def test_a_fully_specified_quote_yields_every_figure(comparable_quote, rutf, tender_2000_cartons):
    f = compute_figures(comparable_quote, rutf, tender_2000_cartons)
    ...
    assert f.landed_total_for_tender_quantity == Money(Decimal("100000.00"))
    assert f.usd_per_child_treated == Money(Decimal("50.00"))


def test_an_unstated_pack_spec_blocks_per_sachet_and_per_course(rutf, tender_2000_cartons):
...
def test_a_confirmed_trade_item_states_the_pack_spec(rutf, tender_2000_cartons, item_150):
    ...
    f = compute_figures(q, rutf, tender_2000_cartons, item=item_150)
    assert f.usd_per_base_unit.amount == Decimal("50.00") / Decimal("150")


def test_a_confirmed_item_overrides_the_commodity_when_they_disagree(rutf, tender_2000_cartons, item_144):
    ...
    f = compute_figures(q, rutf, tender_2000_cartons, item=item_144)
    assert rutf.base_per_pack == 150
    assert f.usd_per_base_unit.amount == Decimal("50.00") / Decimal("144")


def test_a_quote_claiming_a_confirmed_item_without_one_supplied_is_unconfirmed(rutf, tender_2000_cartons):
    ...
    f = compute_figures(q, rutf, tender_2000_cartons, item=None)
    assert "pack spec" in _reasons(f.usd_per_base_unit)


def test_the_catalogue_pack_spec_is_never_substituted(rutf, tender_2000_cartons):
...
def test_a_per_sachet_quote_normalises_up_to_a_carton(rutf, tender_2000_cartons):
...
def test_a_quantity_basis_over_the_tender_blocks_the_tender_total(rutf, tender_2000_cartons):
...
def test_unspecified_freight_blocks_every_landed_figure(rutf, tender_2000_cartons):
    q = quote(freight_basis="not_specified")
    f = compute_figures(q, rutf, tender_2000_cartons)
    assert "freight" in _reasons(f.landed_total_for_tender_quantity)
    assert "freight" in _reasons(f.landed_total_as_quoted)


def test_excluded_freight_with_an_amount_is_added_in(rutf, tender_2000_cartons):
    q = quote(freight_basis="excluded", freight_amount="2000.00")
    f = compute_figures(q, rutf, tender_2000_cartons)
    assert f.landed_total_for_tender_quantity == Money(Decimal("102000.00"))


def test_excluded_freight_without_an_amount_is_unconfirmed(rutf, tender_2000_cartons):
    q = quote(freight_basis="excluded", freight_amount=None)
    f = compute_figures(q, rutf, tender_2000_cartons)
    assert "freight" in _reasons(f.landed_total_for_tender_quantity)


def test_excluded_duties_without_an_amount_is_unconfirmed(rutf, tender_2000_cartons):
    q = quote(duties_basis="excluded", duties_amount=None)
    f = compute_figures(q, rutf, tender_2000_cartons)
    assert "duties" in _reasons(f.landed_total_for_tender_quantity)


def test_a_non_usd_quote_without_a_rate_blocks_everything(rutf, tender_2000_cartons):
...
def test_a_non_usd_quote_with_a_rate_converts(rutf, tender_2000_cartons):
...
def test_a_commodity_without_a_course_definition_blocks_per_course(
    comparable_quote, rutf_without_course, tender_2000_cartons
):
    f = compute_figures(comparable_quote, rutf_without_course, tender_2000_cartons)
...
def test_the_missing_course_definition_reason_uses_the_commodity_own_noun(tender_2000_cartons):
    ...
    f = compute_figures(q, scale, tender_2000_cartons)
    reason = _reasons(f.usd_per_course)
    assert "units per course" in reason
    assert "sachets" not in reason


def test_a_missing_amount_blocks_everything(rutf, tender_2000_cartons):
    q = quote(as_quoted_amount=None)
    f = compute_figures(q, rutf, tender_2000_cartons)
    assert "amount" in _reasons(f.usd_per_pack_normalized)


def test_reasons_accumulate_when_several_facts_are_missing(rutf, tender_2000_cartons):
...
def test_the_catalogue_unit_weight_is_never_substituted(rutf, tender_2000_cartons):
...
def test_a_confirmed_item_states_the_unit_weight_when_the_quote_does_not(rutf, tender_2000_cartons, item_100g):
    ...
    f = compute_figures(q, rutf, tender_2000_cartons, item=item_100g)
...
def test_a_per_tonne_quote_against_a_tonne_basis_is_exact(rutf, tender_2000_cartons):
    q = quote(
        as_quoted_unit="per_metric_tonne",
        as_quoted_amount="5000.00",
        quantity_basis="27.6",
...
def test_a_lot_total_quote_landed_total_is_exact(rutf, tender_2000_cartons):
    q = quote(as_quoted_unit="per_lot_total", as_quoted_amount="100000.00")
    f = compute_figures(q, rutf, tender_2000_cartons)
    assert f.landed_total_as_quoted == Money(Decimal("100000.00"))


def test_a_missing_quantity_basis_names_the_missing_fact_not_the_word_none(rutf, tender_2000_cartons):
