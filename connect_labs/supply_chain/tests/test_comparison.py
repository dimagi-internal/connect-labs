...
def test_two_complete_quotes_are_both_comparable_and_ranked(rutf, tender_2000_cartons):
    quotes = [
        quote(supplier_id=1, as_quoted_amount="50.00"),
        quote(supplier_id=2, as_quoted_amount="52.00"),
    ]
    comparison = compare_tender(tender_2000_cartons, rutf, quotes, suppliers())
...
def test_an_incomplete_quote_is_blocked_not_ranked(rutf, tender_2000_cartons):
    quotes = [
        quote(supplier_id=1, as_quoted_amount="50.00"),
...
def test_a_column_is_rankable_over_the_comparable_subset(rutf, tender_2000_cartons):
...
def test_a_blocked_row_keeps_the_figures_it_does_have(rutf, tender_2000_cartons):
...
def test_column_labels_come_from_the_commodity_not_a_hardcoded_noun(rutf, tender_2000_cartons):
    comparison = compare_tender(tender_2000_cartons, rutf, [quote(supplier_id=1)], suppliers())
...
def test_nothing_is_rankable_when_every_candidate_is_blocked(rutf, tender_2000_cartons):
    quotes = [quote(supplier_id=2, pack_spec_source="not_stated", base_per_pack_stated=None)]
    comparison = compare_tender(tender_2000_cartons, rutf, quotes, suppliers())
...
def test_voided_and_superseded_quotes_are_excluded(rutf, tender_2000_cartons):
    quotes = [
        quote(supplier_id=1, as_quoted_amount="50.00"),
...
def test_every_blocked_row_carries_its_outstanding_questions(rutf, tender_2000_cartons):
    quotes = [quote(supplier_id=2, pack_spec_source="not_stated", base_per_pack_stated=None)]
    comparison = compare_tender(tender_2000_cartons, rutf, quotes, suppliers())
    assert any(f.key == "pack_spec" for f in comparison.blocked[0].questions)


def test_a_confirmed_trade_item_makes_a_quote_comparable(rutf, tender_2000_cartons, item_144):
    ...
    comparison = compare_tender(tender_2000_cartons, rutf, quotes, suppliers(), items_by_id={12: item_144})
    assert comparison.comparable_count == 1
    assert comparison.comparable[0].figures["usd_per_base_unit"].amount == Decimal("50.00") / Decimal("144")


def test_the_snapshot_is_json_serialisable_and_keeps_the_reasons(rutf, tender_2000_cartons):
    import json

    quotes = [quote(supplier_id=2, pack_spec_source="not_stated", base_per_pack_stated=None)]
    snapshot = compare_tender(tender_2000_cartons, rutf, quotes, suppliers()).to_snapshot()
    text = json.dumps(snapshot)
    assert "pack spec" in text


def test_the_cheaper_landed_total_sorts_first(rutf, tender_2000_cartons):
    quotes = [
        quote(supplier_id=1, as_quoted_amount="50.00"),
        quote(supplier_id=2, as_quoted_amount="48.00"),
    ]
    comparison = compare_tender(tender_2000_cartons, rutf, quotes, suppliers())
    assert comparison.ranked_by == "landed_total_for_tender_quantity"
...
def test_ranked_by_is_none_when_the_tender_has_no_line_for_the_commodity(rutf):
    """Ruling 22: the old "usd_per_pack_normalized" fallback was dead code --
    a tender with no line for this commodity makes landed_total_for_tender_quantity
    ...
    tender_no_line = wrap(Tender, {"lines": []})
    comparison = compare_tender(tender_no_line, rutf, [quote(supplier_id=1)], suppliers())
    assert comparison.comparable_count == 0
    assert comparison.ranked_by is None


def test_provisional_is_true_when_anything_is_blocked(rutf, tender_2000_cartons):
    quotes = [
        quote(supplier_id=1, as_quoted_amount="50.00"),
        quote(supplier_id=2, pack_spec_source="not_stated", base_per_pack_stated=None),
    ]
...
def test_provisional_is_false_when_nothing_is_blocked(rutf, tender_2000_cartons):
    quotes = [
        quote(supplier_id=1, as_quoted_amount="50.00"),
        quote(supplier_id=2, as_quoted_amount="52.00"),
    ]
...
def test_ranked_by_and_provisional_are_present_in_the_snapshot(rutf, tender_2000_cartons):
    quotes = [
        quote(supplier_id=1, as_quoted_amount="50.00"),
        quote(supplier_id=2, pack_spec_source="not_stated", base_per_pack_stated=None),
    ]
    snapshot = compare_tender(tender_2000_cartons, rutf, quotes, suppliers()).to_snapshot()
    assert snapshot["ranked_by"] == "landed_total_for_tender_quantity"
    assert snapshot["provisional"] is True


def test_blocked_by_dedupes_a_supplier_with_two_blocked_quotes(rutf, tender_2000_cartons):
    quotes = [
        quote(supplier_id=2, pack_spec_source="not_stated", base_per_pack_stated=None),
        quote(supplier_id=2, pack_spec_source="not_stated", base_per_pack_stated=None),
    ]
...
def test_our_own_missing_ration_table_does_not_block_a_supplier(rutf_without_course, tender_2000_cartons):
    ...
    """
    comparison = compare_tender(tender_2000_cartons, rutf_without_course, [quote(supplier_id=1)], suppliers())
...
def test_the_uncomputable_columns_are_reported_once_as_ours(rutf_without_course, tender_2000_cartons):
...
def test_a_figure_missing_on_only_one_row_is_that_suppliers_gap_not_ours(rutf, tender_2000_cartons):
...
def test_a_column_nobody_can_compute_is_not_rankable(rutf_without_course, tender_2000_cartons):
    ...
    assert by_key["landed_total_for_tender_quantity"].rankable is True
