...
# ---- 1. a partner's link records goods received for any order to its store --


class TestAPartnersLinkCoversOrdersDeliveredToItsStore:
    def test_an_order_placed_after_the_link_can_be_received_through_it(self, da, world):
        cover = _cover(da, world, received=False)
        scope = service.scope_for(world["partner_link"])
    ...
    def test_an_order_delivered_elsewhere_stays_out_of_scope(self, da, world):
        other = op(da, "org_upsert", data={"slug": "other", "name": "Another partner"})
        elsewhere = op(
            da,
            "supply_point_upsert",
            data={
        ...
        order = op(
            ...
            data={
                ...
                "reference": "ELSEWHERE-1",
                "status": "placed",
                "consideration": "bundled",
                "quantity": "10",
                "quantity_unit": "packet",
                "delivery_supply_point_id": elsewhere["id"],
                "source": "we_recorded",
            },
        )
        scope = service.scope_for(world["partner_link"])
        assert not scope.contracts.filter(pk=order["id"]).exists()
        with pytest.raises(service.OutOfScope):
            service.submit(
                world["partner_link"],
                "record_receipt",
                {
                    "contract": Contract.objects.get(pk=order["id"]),
                    "supply_point": SupplyPoint.objects.get(pk=world["store"]["id"]),
    ...
    def test_the_distributors_link_does_not_gain_the_partners_orders(self, da, world):
        cover = _cover(da, world, received=False)
...
class TestTheOrderPage:
    def test_the_feed_uses_the_link_pages_words(self, scoped, played):
        text = _visible(_page(scoped, "order_detail", played["order"]["id"]))
        assert "Record a dispatch —" in text
        assert "Record goods received —" in text
        assert "Record shipment" not in text
        assert "Record receipt" not in text

    def test_a_name_ending_in_s_takes_a_bare_apostrophe(self, scoped, played):
        text = _visible(_page(scoped, "order_detail", played["order"]["id"]))
        assert "Harmattan Health Supplies' update link" in text
...
class TestTheStockPage:
    ...
    def test_the_send_column_is_suggested_resupply(self, scoped, played):
        body = _page(scoped, "stock")
        headers = [_visible(h) for h in re.findall(r"<th[^>]*>(.*?)</th>", body, re.S)]
        assert "Suggested resupply" in headers
