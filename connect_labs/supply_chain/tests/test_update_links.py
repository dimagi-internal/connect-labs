...
from connect_labs.supply_chain.models import Contract, Document, Movement, Receipt, Shipment, StockCount
...
class TestWritesGoThroughTheOrdinaryOperations:
    ...
    def test_confirming_a_payment_records_the_suppliers_statement_against_the_payment(self, da, issued, world):
        invoice = op(
            da,
            "invoice_record",
        ...
        document = Document.objects.get(payment_id=payment["id"])
        assert (document.source, document.recorded_by_org_id) == ("supplier_reported", world["eha"]["id"])
