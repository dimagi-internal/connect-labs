...
def _confirm_payment(scope, data):
    ...
    # TODO(G8): `Payment.confirmed_by_payee_on` is being added by a parallel
    # change. Once it exists, with an operation that sets it, this action
    # should call that operation (with the same provenance) instead of -- or
    # as well as -- attaching the statement below. Until then the supplier's
    # confirmation is kept as a document against the payment, which is a real
    # record of what they said and is visible on the order page.
    statement = (
        f"{scope.link.org.name} confirms receiving payment {payment.pk} "
        f"({payment.amount} {payment.currency}, paid {payment.paid_on.isoformat()}) "
        f"on {received_on.isoformat()}."
    )
    if reference:
        statement += f" Their reference: {reference}."
    document = {
        "kind": "other",
        "title": f"Payment received — confirmed by {scope.link.org.name}",
        "filename": f"payment-{payment.pk}-received.txt",
        "content_type": "text/plain",
        "content_base64": base64.b64encode(statement.encode()).decode(),
        "payment_id": payment.pk,
        **_provenance(scope.link),
    }
    return "document_attach", {"data": document}, Action.CREATE
