...
const LABS =
  process.env.CAMPAIGN_LABS_BASE || 'https://labs.connect.dimagi.com';
const STATE = path.join(os.homedir(), '.ace', 'labs-session.json');

async function main() {
  if (!fs.existsSync(STATE)) {
    throw new Error(
      `no ${STATE} — run /ace:labs-login first to establish the CommCare-HQ session`,
    );
  }
  ...
  try {
    ...
    await page.goto(`${LABS}/campaign/login/initiate/`, {
    ...
    await page.waitForURL(
      (u) => {
        const url = new URL(u);
        return (
          (url.hostname === new URL(LABS).hostname &&
    ...
    if (new URL(page.url()).hostname.endsWith('commcarehq.org')) {
      ...
      if ((await allow.count()) > 0) {
        await Promise.all([
          page.waitForURL((u) => new URL(u).pathname.startsWith('/campaign'), {
            timeout: 45_000,
          }),
          allow.click(),
        ]);
    ...
    if (onLogin)
      throw new Error(
        'still on /campaign/login after OAuth — consent or whitelist failed',
      );
    ...
    const sid = cookies.find(
      (c) => c.name === 'sessionid' && c.domain.includes(host),
    );
    ...
    process.stdout.write(
      JSON.stringify({
        sessionDomain: sid.domain,
        sessionName: sid.name,
        sessionValue: sid.value,
        host,
      }) + '\n',
    );
  } finally {
