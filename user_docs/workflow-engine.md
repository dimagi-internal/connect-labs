...
### Viewing audit runs for an opportunity

The audit page for an opportunity shows a **single card listing every audit run**. Click any run in the list to expand it in place — the full per-field-worker breakdown appears directly beneath that run without navigating away from the page. The breakdown shows each FLW's MUAC and Other audit counts, image counts, pass/fail/pending results, and AI-flagged counts, and each FLW row links through to their individual audit detail.

Click the same run again to collapse it. You can expand one run at a time to review its detail, then expand a different run to compare, all within the same card.
...
Each opportunity row in the Program Audit Creator has an expand chevron (›). Clicking it opens an inline breakdown of that opportunity's full audit detail — every field worker listed with their MUAC and Other audit counts, image counts, pass/fail/pending results
