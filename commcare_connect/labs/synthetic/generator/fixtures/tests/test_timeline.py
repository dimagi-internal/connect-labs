...
from commcare_connect.labs.synthetic.generator.fixtures.manifest import FlwPersona, MeanStddev, Timeline
from commcare_connect.labs.synthetic.generator.fixtures.timeline import VisitSlot, expand_visit_schedule
...
def test_weekday_distribution_respects_weights():
    timeline = Timeline(
        start_date=dt.date(2026, 5, 4),
        end_date=dt.date(2026, 6, 1),
        weeks=4,
        visit_cadence_per_week_per_flw=MeanStddev(mean=20, stddev=0),
    )
    persona = FlwPersona(
        id="x",
        archetype="steady",
        accuracy_distribution=MeanStddev(mean=0.8, stddev=0),
        completeness_distribution=MeanStddev(mean=0.8, stddev=0),
        flag_rate=0.0,
    )
    # all weight on Monday (index 0)
    slots = expand_visit_schedule(timeline, [persona], random_seed=1, day_of_week=[1, 0, 0, 0, 0, 0, 0])
    assert {s.visit_date.weekday() for s in slots} == {0}
