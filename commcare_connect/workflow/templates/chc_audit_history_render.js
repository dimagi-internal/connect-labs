...
var CHC_METRICS = [
  { key: 'camping_visit_building_ratio',         label: 'Camping',                      full: 'Camping (Visit:Building Ratio)',                       flagFn: function(v) { return v > 0; } },
  { key: 'gender_ratio_deviation',               label: 'Gender Ratio',                 full: 'Gender Ratio Deviation',                               flagFn: null },
  { key: 'muac_photo_compliance',                label: 'MUAC Photo',                   full: 'MUAC Photo Compliance',                                flagFn: function(v) { return v < 90; } },
  { key: 'age_heaping',                          label: 'Age Heaping',                  full: 'Age Heaping',                                          flagFn: function(v) { return v > 15; } },
  { key: 'wa_coverage_to_visit_ratio',           label: 'WA Coverage:Visit',            full: 'WA Coverage to Visit Ratio',                           flagFn: function(v) { return v > 0.13; } },
  { key: 'inaccessible_wa_rate_early_warning',   label: 'Inaccess. WA (Early)',         full: 'Inaccessible WA Rate – Early Warning',             flagFn: null },
  { key: 'inaccessible_wa_rate_last_wag',        label: 'Inaccess. WA (Last)',          full: 'Inaccessible WA Rate – Last Completed WAG',        flagFn: null },
  { key: 'vaccine_rate',                         label: 'Vaccine Rate',                 full: 'Vaccine Rate',                                         flagFn: null },
  { key: 'vaccine_card_photo_compliance',        label: 'Vaccine Card',                 full: 'Vaccine Card Photo Compliance',                        flagFn: null },
  { key: 'muac_distribution_pattern_index',      label: 'MDPI',                         full: 'MUAC Distribution Pattern Index (MDPI)',                flagFn: null },
];
...
function chcDateRange(start, end) {
  ...
  function yr(d) { return d.split('-')[0]; }
...
function ChcSortTh(props) {
  ...
  return ce('th', {
    className: 'px-3 py-2 text-left text-xs font-semibold uppercase tracking-wider cursor-pointer select-none whitespace-nowrap ' +
      (active ? 'bg-green-900 text-white' : 'bg-green-800 text-green-100 hover:bg-green-700'),
    onClick: function() {
      if (active) {
  ...
  }, props.label, ' ', ce('span', { className: 'opacity-60 text-xs' }, icon));
}

function ChcPctCell(props) {
  var pct = props.den > 0 ? Math.round((props.num / props.den) * 100) : null;
  if (pct == null) return ce('td', { className: 'px-3 py-2 text-right text-gray-400 text-sm' }, '—');
  var good = props.higherIsBetter !== false ? pct >= 70 : pct <= 30;
  return ce('td', { className: 'px-3 py-2 text-right text-sm tabular-nums' },
...
function ChcAuditHistory(props) {
  ...
  function handleSort(col, dir) { setSort({ col: col, dir: dir }); }
  ...
  var enriched = React.useMemo(function() {
    return filtered.map(function(r) {
      var entries = entryRows.filter(function(e) {
        return String(e.report_id) === String(r.report_id) ||
               String(e.report_id) === String(r.id);
      });
      var passedN = entries.filter(function(e) {
        return !e.flagged && e.flagged !== 1 && e.flagged !== '1' && e.flagged !== true;
      }).length;
      var periodTasks = taskRows.filter(function(t) {
        return chcIsInPeriod(t.date_created, r.period_start, r.period_end) &&
          (oppFilter === 'all' || String(r.opportunity_id) === oppFilter);
      });
      var workerSet = {};
      entries.forEach(function(e) { if (e.username) workerSet[e.username] = true; });
      var periodWorkerTasks = periodTasks.filter(function(t) { return workerSet[t.username]; });
      var closedTasks = periodWorkerTasks.filter(function(t) {
        return t.task_status === 'closed' || t.task_status === 'completed';
      });
      var pendingWorkers = {};
      periodWorkerTasks.forEach(function(t) {
        if (t.task_status !== 'closed' && t.task_status !== 'completed') {
          pendingWorkers[t.username] = true;
        }
      });
      var durations = closedTasks
        .map(function(t) { return parseFloat(t.duration); })
        .filter(function(d) { return !isNaN(d) && d > 0; });
      var avgMin = durations.length > 0
  ...
  var sorted = React.useMemo(function() {
    ...
    copy.sort(function(a, b) {
      ...
      else if (sort.col === 'time')    { va = a.avgMin || 0; vb = b.avgMin || 0; }
      else if (sort.col === 'runby')   { va = r.completed_by_username || ''; vb = s.completed_by_username || ''; }
      else { va = ''; vb = ''; }
  ...
  return ce('div', { className: 'p-4 space-y-3' },
    // Filter bar
    ce('div', { className: 'flex items-center gap-3 flex-wrap' },
      ...
      ce('select', {
        ...
        value: oppFilter,
        onChange: function(e) { setOppFilter(e.target.value); },
      },
        ce('option', { value: 'all' }, 'All Opportunities'),
        oppIds.map(function(id) { return ce('option', { key: id, value: String(id) }, 'Opp #' + id); })
      ),
        ...
        ce('table', { className: 'w-full border-collapse bg-white text-sm' },
          ce('thead', null,
            ce('tr', null,
              ...
              ce(ChcSortTh, Object.assign({ colKey: 'time',         label: 'Time to Task Completed' }, thProps)),
              ce(ChcSortTh, Object.assign({ colKey: 'runby',        label: 'Run By' }, thProps))
            )
          ),
          ce('tbody', null,
            sorted.length === 0
              ? ce('tr', null, ce('td', { colSpan: 9, className: 'px-4 py-8 text-center text-gray-400 text-sm' }, 'No audit reports found'))
              : sorted.map(function(row, i) {
                  ...
                  return ce('tr', { key: i, className: 'border-b border-gray-100 hover:bg-gray-50' + (!completed ? ' text-gray-500' : '') },
                    ...
                    ce('td', { className: 'px-3 py-2 text-right tabular-nums text-sm' },
                      row.avgMin != null ? row.avgMin + ' min' : '—'
                    ),
                    ce('td', { className: 'px-3 py-2 text-sm' }, r.completed_by_username || '—')
                  );
                })
          )
        )
...
function ChcMetricDetail(props) {
  ...
  var filteredEntries = React.useMemo(function() {
    ...
    if (oppFilter !== 'all') {
      var ridsForOpp = {};
      reportRows.filter(function(r) { return String(r.opportunity_id) === oppFilter; })
        .forEach(function(r) { ridsForOpp[String(r.report_id || r.id)] = true; });
      rows = rows.filter(function(e) {
        return ridsForOpp[String(e.report_id)];
      });
    }
    if (repFilter !== 'all') {
      rows = rows.filter(function(e) { return String(e.report_id) === repFilter; });
    }
    return rows;
  ...
  // Column flag totals
  var colTotals = React.useMemo(function() {
  ...
  return ce('div', { className: 'p-4 space-y-3' },
    ...
    ce('div', { className: 'flex items-center gap-3 flex-wrap' },
      ...
      ce('select', {
        className: 'text-sm border border-gray-300 rounded px-2 py-1.5 bg-white',
        value: repFilter,
        onChange: function(e) { setRepFilter(e.target.value); },
      },
        ce('option', { value: 'all' }, 'All Completed Audits'),
        completedReports.map(function(r, i) {
          return ce('option', { key: i, value: String(r.report_id || r.id) },
            'Opp #' + r.opportunity_id + ' · ' + chcDateRange(r.period_start, r.period_end)
          );
        })
      ),
      ce('span', { className: 'text-xs font-semibold uppercase tracking-wider text-gray-500' }, 'Opportunity'),
      ...
      ce('span', { className: 'text-xs text-gray-400' }, flwRows.length + ' worker' + (flwRows.length !== 1 ? 's' : ''))
    ),
    // Pivot table
    ce('div', { className: 'rounded-lg overflow-hidden shadow-sm border border-gray-200' },
      ce('div', { className: 'overflow-x-auto' },
        ce('table', { className: 'border-collapse bg-white text-xs' },
          ce('thead', null,
            ce('tr', null,
              ce('th', { className: 'sticky left-0 z-10 bg-green-800 text-green-100 px-3 py-2 text-left text-xs font-semibold uppercase tracking-wider whitespace-nowrap min-w-40', style: { position: 'sticky', left: 0 } }, 'Connect Worker'),
              CHC_METRICS.map(function(m) {
                return ce('th', {
                  key: m.key,
                  title: m.full,
                  className: 'bg-green-800 text-green-100 px-2 py-2 text-center font-semibold uppercase tracking-wider min-w-20 leading-tight',
                  style: { whiteSpace: 'normal', maxWidth: '80px' },
                }, m.label);
              }),
              ce('th', { className: 'bg-green-900 text-white px-3 py-2 text-center text-xs font-semibold uppercase tracking-wider' }, 'Flags')
            )
          ),
          ce('tbody', null,
            flwRows.length === 0
              ? ce('tr', null, ce('td', { colSpan: CHC_METRICS.length + 2, className: 'px-4 py-8 text-center text-gray-400' }, 'No data for current filters'))
              : flwRows.map(function(row, i) {
                  ...
                  var cells = CHC_METRICS.map(function(m) {
                    ...
                    if (rawVal == null) {
                      return ce('td', { key: m.key, className: 'border border-gray-200 px-2 py-1.5 text-center text-gray-400' }, 'N/A');
                    }
                    ...
                    return ce('td', {
                      ...
                      className: 'border border-gray-200 px-2 py-1.5 text-center tabular-nums ' +
                        (isFlagged ? 'bg-amber-100 text-amber-800 font-semibold' : ''),
                    }, isNaN(v) ? String(rawVal) : v);
                  });
                  return ce('tr', { key: i, className: 'hover:bg-gray-50' },
                    ce('td', { className: 'border border-gray-200 px-3 py-1.5 text-left font-medium whitespace-nowrap sticky left-0 bg-white z-10', style: { position: 'sticky', left: 0 } }, row.username),
                    cells,
                    ce('td', { className: 'border border-gray-200 px-2 py-1.5 text-center font-bold bg-green-900 text-white' }, rowFlags || '0')
                  );
                })
          ),
          ce('tfoot', null,
            ce('tr', null,
              ce('td', { className: 'border border-gray-200 px-3 py-1.5 text-left font-bold text-xs uppercase bg-green-50 text-green-800 sticky left-0', style: { position: 'sticky', left: 0 } }, 'Metric totals'),
              colTotals.map(function(n, i) {
                return ce('td', {
                  key: i,
                  className: 'border border-gray-200 px-2 py-1.5 text-center font-bold text-xs ' +
                    (n > 0 ? 'bg-amber-100 text-amber-800' : 'bg-green-50 text-green-700'),
                }, n || '0');
              }),
              ce('td', { className: 'border border-gray-200 px-2 py-1.5 text-center font-bold bg-green-900 text-white' }, grandTotal)
            )
          )
        )
      )
...
function ChcFLWLongitudinal(props) {
  ...
  var cycles = React.useMemo(function() {
    var filteredReports = oppFilter === 'all'
      ? reportRows
      : reportRows.filter(function(r) { return String(r.opportunity_id) === oppFilter; });
    var completedReports = filteredReports.filter(function(r) { return r.status === 'completed'; });
    ...
    return flwList.map(function(username) {
      ...
      var cycleStats = cycles.map(function(cycle) {
        ...
        var flagCount = entries.filter(function(e) {
          return e.flagged === true || e.flagged === 1 || e.flagged === '1';
        }).length;
        var metricFlags = 0;
        entries.forEach(function(e) {
          ...
          CHC_METRICS.forEach(function(m) {
            var v = res[m.key];
            if (v != null && m.flagFn && m.flagFn(parseFloat(v))) metricFlags++;
          });
        });
        ...
        return {
          ...
          label: chcDateRange(cycleReports[0].period_start, cycleReports[0].period_end),
        };
      });
  ...
  return ce('div', { className: 'p-4 space-y-3' },
    ...
    // Legend
    ce('div', { className: 'flex gap-3 text-xs text-gray-500 flex-wrap items-center' },
      ['0 flags', '1 flag', '2 flags', '3+ flags'].map(function(lbl, i) {
        var bg = ['bg-white border-gray-200', 'bg-amber-50 border-amber-200', 'bg-amber-100 border-amber-300', 'bg-amber-200 border-amber-400'][i];
        return ce('span', { key: i, className: 'flex items-center gap-1' },
          ce('span', { className: 'inline-block w-3 h-3 rounded border ' + bg }),
          lbl
        );
      }),
      ...
      : ce('div', { className: 'rounded-lg overflow-hidden shadow-sm border border-gray-200' },
          ce('div', { className: 'overflow-x-auto' },
            ce('table', { className: 'border-collapse bg-white text-xs w-full' },
              ce('thead', null,
                ce('tr', null,
                  ce('th', { className: 'sticky left-0 z-10 bg-green-900 text-white px-3 py-2 text-left text-xs font-semibold uppercase tracking-wider whitespace-nowrap min-w-40', style: { position: 'sticky', left: 0 } }, 'Connect Worker'),
                  ce('th', { className: 'bg-green-900 text-white px-3 py-2 text-center text-xs font-semibold uppercase tracking-wider whitespace-nowrap min-w-20' }, 'Total Flags'),
                  cycles.map(function(c, i) {
                    var r = c.reports[0];
                    return ce('th', { key: i, className: 'bg-green-800 text-green-100 px-3 py-2 text-center font-semibold min-w-32' },
              ...
              ce('tbody', null,
                flwCycleData.map(function(row, i) {
                  return ce('tr', { key: i, className: 'border-b border-gray-100' },
                    ce('td', { className: 'border-r border-gray-200 px-3 py-2 font-medium whitespace-nowrap sticky left-0 bg-gray-50 z-10', style: { position: 'sticky', left: 0 } }, row.username),
                    ce('td', { className: 'border-r border-gray-200 px-3 py-2 text-center font-bold ' + (row.totalFlags > 0 ? 'bg-green-50 text-green-800' : 'bg-white text-gray-400') }, row.totalFlags),
                    row.cycles.map(function(cs, j) {
                      if (!cs) {
                        return ce('td', { key: j, className: 'border-r border-gray-200 px-3 py-2 text-center text-gray-300' }, '—');
                      }
                      var mPct = cs.totalMetrics > 0 ? Math.round(((cs.totalMetrics - cs.metricFlags) / cs.totalMetrics) * 100) : 100;
                      var tPct = cs.totalTasks > 0 ? Math.round((cs.closedTasks / cs.totalTasks) * 100) : null;
                      return ce('td', { key: j, className: 'border-r border-gray-200 px-3 py-2 ' + severityCls(cs.metricFlags) },
                        ce('div', { className: 'flex flex-col items-center gap-0.5' },
                          ce('span', { className: 'font-semibold text-green-800' }, mPct + '% passed'),
                          ce('div', { className: 'w-16 h-1.5 bg-gray-200 rounded overflow-hidden' },
                            ce('div', { className: 'h-full bg-green-600 rounded', style: { width: mPct + '%' } })
                          ),
                          tPct != null
                            ? ce('span', { className: 'text-amber-700' }, tPct + '% tasks')
                            : ce('span', { className: 'text-gray-400' }, 'no tasks')
                        )
                      );
                    })
                  );
...
function WorkflowContent(props) {
  ...
  return ce('div', { className: 'min-h-screen bg-gray-50' },
    // Header
    ce('div', { className: 'bg-green-900 text-white px-6 py-4' },
      ...
      ce('div', { className: 'flex gap-2 mt-2 flex-wrap' },
        oppIds.map(function(id) {
          return ce('span', { key: id, className: 'text-xs px-2 py-0.5 bg-white/10 border border-white/20 rounded' }, 'Opp #' + id);
        })
      )
    ),
    // Tabs
    ...
    // Tab content
    activeTab === 0 && ce(ChcAuditHistory, { reportRows: reportRows, entryRows: entryRows, taskRows: taskRows, oppIds: oppIds }),
    activeTab === 1 && ce(ChcMetricDetail,  { reportRows: reportRows, entryRows: entryRows, oppIds: oppIds }),
    activeTab === 2 && ce(ChcFLWLongitudinal, { reportRows: reportRows, entryRows: entryRows, taskRows: taskRows, workers: workers, oppIds: oppIds })
  );
}
