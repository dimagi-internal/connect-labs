// =============================================================================
// V3 Pipeline → Dashboard Adapter
// -----------------------------------------------------------------------------
// Replaces v2's celery-driven `run_workflow_job` task. v2 shipped per-FLW
// dashboard data (gps_data, followup_data, quality_metrics, overview_data,
// performance_data) as the celery task RESULT — published into Redis as a
// task-meta blob, which OOM'd ElastiCache on production-scale opportunities.
//
// v3 sidesteps celery entirely: pipelines run via SQL during the SSE pipeline
// stream, populate `pipelines.<alias>.rows` on the JSX runner, and this
// adapter derives the v2-shaped results object client-side. No celery
// publish, no Redis blob.
//
// The adapter intentionally produces the SAME shape v2's UI consumes so the
// rest of this 8400-line render file stays unchanged — only the data-loading
// boundary shifts. See `runAnalysis` below for the call site.
// =============================================================================

// MBW visit-type bookkeeping. Mirrors followup_analysis.py constants exactly
// so the JS adapter stays drift-free against v1's classification logic.
var MBW_VISIT_TYPE_KEYS = [
  'anc',
  'postnatal',
  'week1',
  'month1',
...
var MBW_FORM_NAME_TO_VISIT_TYPE = {
  'ANC Visit': 'ANC Visit',
...
function _v3PipelineRows(pipelines, alias) {
  var p = pipelines && pipelines[alias];
  if (!p) return [];
  // Pipeline rows arrive as either flat dicts or {custom_fields} wrappers
  // depending on terminal stage. Normalize so the adapter sees one shape.
  return (p.rows || []).map(function (r) {
    var fields = (r && r.computed) || (r && r.custom_fields) || r || {};
    // Always carry through base columns (visit_id, username, visit_date,
    // status) regardless of where they sit on the row object.
    return Object.assign({}, fields, {
      _username: r.username || fields.username || '',
      _visit_date: r.visit_date || fields.visit_date || '',
      _opportunity_id: r.opportunity_id || fields.opportunity_id || null,
    });
  });
}

function _v3MedianOf(values) {
  var sorted = values
    .filter(function (v) {
...
function _v3BuildOverviewData(visitsRows, todayStr) {
  ...
  visitsRows.forEach(function (r) {
    ...
    if (!flw) return;
    if (typeof r.mother_count === 'number') motherCounts[flw] = r.mother_count;
...
function _v3BuildQualityMetrics(visitsRows) {
  // Per-FLW quality leaves, all sourced from visits pipeline's two-pass
  // aggregations (parity_*, phone_dup_share, age_concentration_*). The
  // shape exactly matches v1's `compute_overview_quality_metrics` so v2's
  // FLW table and modals consume it unchanged.
  var quality = {};
  visitsRows.forEach(function (r) {
    var flw = (r._username || '').toLowerCase();
    if (!flw) return;
    var pms = r.parity_mode_share;
    var pds = r.parity_dup_share;
    var pct = function (s) {
      return s == null ? 0 : Math.round(s * 100);
    };
    quality[flw] = {
      phone_dup_pct: pct(r.phone_dup_share),
      // anc_pnc_* leaves are not yet expressible in v3's primitives —
      // they need cross-form-type per-mother extraction. UI shows blanks.
      anc_pnc_same_date_count: 0,
      anc_pnc_denominator: 0,
      parity_concentration: {
        mode_pct: pct(pms),
        mode_value: r.parity_mode_value || '',
        pct_duplicate: pct(pds),
      },
      age_concentration: {
        mode_pct: pct(r.age_concentration_mode_share),
        mode_value: r.age_concentration_mode_value || '',
        pct_duplicate: pct(r.age_concentration_dup_share),
      },
      // age_equals_reg_pct needs DOB + reg-date date-arithmetic comparison
      // also not yet in v3 primitives. UI shows blank.
      age_equals_reg_pct: 0,
    };
  });
  return quality;
}
...
function _v3BuildGpsData(visitsGpsRows, flwNameMap) {
  ...
  var allDates = [];
  var totalFlagged = 0;
  visitsGpsRows.forEach(function (r) {
    var flw = (r._username || '').toLowerCase();
    if (!flw) return;
    if (!byFlw[flw]) {
      byFlw[flw] = {
  ...
  var summaries = Object.keys(byFlw).map(function (flw) {
    var b = byFlw[flw];
    var distMed = _v3MedianOf(b.distances_m);
    if (distMed != null) medianMeters[flw] = Math.round(distMed);
    // Minutes-between-consecutive-visits: ms diffs of sorted timestamps,
    // converted to minutes; matches v1's compute_median_minutes_per_visit.
    var ts = b.timestamps_ms.slice().sort(function (a, b) {
      return a - b;
    });
    var gaps = [];
    ...
    var avgKm = b.distances_m.length
      ? sumDist / b.distances_m.length / 1000
      : null;
  ...
  return {
    ...
    median_minutes_by_flw: medianMinutes,
...
function _v3BuildFollowupData(
  ...
  var motherToFlw = {};
  var motherLastTs = {};
  var visitsByMotherType = {}; // {mid: {visit_type: visit_date}}
  visitsGpsRows.forEach(function (r) {
    var mid = r.mother_case_id;
    var flw = (r._username || '').toLowerCase();
    if (!mid || !flw) return;
    var dt = r.visit_datetime || r._visit_date;
    var ts = dt ? Date.parse(dt) : 0;
    if (!motherLastTs[mid] || ts > motherLastTs[mid]) {
      motherLastTs[mid] = ts;
      motherToFlw[mid] = flw;
    }
    var formName = (r.form_name || '').trim();
    var visitType = MBW_FORM_NAME_TO_VISIT_TYPE[formName];
    if (visitType) {
      if (!visitsByMotherType[mid]) visitsByMotherType[mid] = {};
      visitsByMotherType[mid][visitType] = (dt + '').slice(0, 10);
    }
  });
  ...
  registrationsRows.forEach(function (reg) {
    ...
    var firstMid = schedules[0].mother_case_id;
    if (!firstMid) return;
  ...
  Object.keys(motherToSchedules).forEach(function (mid) {
    ...
    var motherVisits = visitsByMotherType[mid] || {};
    var visitEntries = schedules.map(function (s) {
  ...
  var flwDrilldown = {};
  Object.keys(flwBuckets).forEach(function (flw) {
...
function buildResultsFromV3Pipelines(
  pipelines,
  instance,
  flwNameMap,
  currentDateStr,
) {
  // Single entry point. Calls each per-section builder and returns the
  // v2-shape `results` dict so `runAnalysis` below can drop it into
  // `dashData` exactly as v2's celery onComplete handler did.
...
function WorkflowUI({
  ...
  // Run analysis from v3 pipelines. v2 dispatched a celery task and streamed
  // its progress; v3 derives the same shape synchronously from `pipelines`
  // props that the SSE pipeline stream already populated. No celery, no
  // task-meta blob in Redis — sidesteps the OOM that v2 hit on large opps.
  var runAnalysis = React.useCallback(
    function () {
      ...
      setJobRunning(true);
      setJobError(null);
      setJobMessages(['Building dashboard from pipeline data...']);
      setDashData(null);
      ...
      try {
        ...
        setJobRunning(false);
        setJobError(
          'Failed to build dashboard from pipelines: ' + (err.message || err),
        );
      }
    },
    [
  ...
  // No celery cleanup needed for v3 — runAnalysis is synchronous now.
  ...
  // OAuth gating happens at the workflow-runner framework level via
  // definition.config.auth_requires (see mbw_monitoring_v3.py DEFINITION).
  // The runner blocks the template from rendering at all when a required
  // service is expired, so the user lands on the framework's
  // "Authorize CommCare HQ" screen within ~1s — before clicking Launch
  // Dashboard, before any pipeline streams. v2's mid-template oauthStatus
