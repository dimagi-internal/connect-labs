...
class ProgramBulkCreatePlansView(LoginRequiredMixin, View):
    ...
    def post(self, request, program_id):
        ...
        boundary_ids = [
            str((p or {}).get("boundary_id") or "").strip()
        ...
        boundary_by_id = {
            b.boundary_id: b
            for b in AdminBoundary.objects.filter(boundary_id__in=wanted)
        }
        ...
        for spec in plans_input:
            ...
            if not boundary_id:
                results.append(
                    {"name": name, "status": "error", "detail": "missing boundary_id"}
                )
                continue
            boundary = boundary_by_id.get(boundary_id)
            if not boundary:
                results.append(
                    {"name": name, "status": "error", "detail": "boundary not found"}
                )
                continue
            try:
                ...
                results.append(
                    {"name": name, "status": "error", "detail": f"bad geometry: {e}"}
                )
                continue
            hulls = {
            ...
            try:
                ...
                logger.exception(
                    "bulk_create: create_plan failed (program=%s, ward=%s)", program_id, name
                )
                results.append(
                    {"name": display_name, "status": "error", "detail": "create_plan failed"}
                )
