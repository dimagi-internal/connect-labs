...
VENV_BIN ?= $(shell \
	for p in \
		"./.venv/bin" \
		"$(HOME)/emdash-projects/connect-labs/.venv/bin" \
		"$(HOME)/venvs/commcare-labs/bin" \
	; do \
...
commit:
	@if [ -z "$(VENV_BIN)" ] || [ ! -x "$(VENV_BIN)/pre-commit" ]; then \
		...
		echo "Tried: ./.venv/bin, ~/emdash-projects/connect-labs/.venv/bin, ~/venvs/commcare-labs/bin" >&2; \
